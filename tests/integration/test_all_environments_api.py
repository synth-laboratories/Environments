...
class TestAllEnvironmentsAPI:
    ...
    def get_test_action_for_env(self, env_name: str) -> Dict[str, Any]:
        ...
        action_map = {
            "Sokoban": {
                "tool_calls": [{"tool": "move", "args": {"direction": "up"}}]
            },
            "CrafterClassic": {
                "tool_calls": [{"tool": "interact", "args": {"action": 2}}]
            },
            "HendryksMath": {
                "tool_calls": [{"tool": "solve", "args": {"answer": "42"}}]
            },
            "Verilog": {
                "tool_calls": [{"tool": "edit", "args": {"content": "module test();\nendmodule"}}]
            },
            "TicTacToe": {
                "tool_calls": [{"tool": "place", "args": {"position": 0}}]
            },
            "NetHack": {
                "tool_calls": [{"tool": "move", "args": {"direction": "north"}}]
            },
            "MiniGrid": {
                "tool_calls": [{"tool": "move", "args": {"action": 2}}]
            },
            "Enron": {
        ...
        return action_map.get(env_name, {
    ...
    async def _test_single_environment_workflow(
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if init_response.status_code != 200:
                raise Exception(f"Initialize failed: {init_response.status_code} - {init_response.text}")
            ...
            try:
                ...
                step_response = await client.post(
                    f"{service_url}/env/{env_name}/step",
                    json=step_payload
                )
                
                if step_response.status_code != 200:
                    raise Exception(f"Step failed: {step_response.status_code} - {step_response.text}")
                ...
                if term_response.status_code != 200:
                    raise Exception(f"Terminate failed: {term_response.status_code} - {term_response.text}")
    ...
    async def test_health_endpoint(self, service_url: str):
        ...
        async with httpx.AsyncClient() as client:
            ...
            expected_envs = ["Sokoban", "CrafterClassic", "MiniGrid"]
    ...
    def test_action_mapping_completeness(self, available_environments: List[str]):
        ...
        for env_name in ["Sokoban", "CrafterClassic", "HendryksMath", "Verilog", 
                        "TicTacToe", "NetHack", "MiniGrid", "Enron"]:
            action = self.get_test_action_for_env(env_name)
    ...
    async def test_concurrent_environment_initialization(
        ...
        async def init_env(env_name: str):
            async with httpx.AsyncClient() as client:
                ...
                return env_name, response.status_code, response.json() if response.status_code == 200 else None
