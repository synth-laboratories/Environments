...
from synth_env.service.app import app
from synth_env.examples.sokoban.units.astar_common import ENGINE_ASTAR
...
PUZZLES = {
    ...
    "one_move": {
        "dim_room": [3, 3],
        "room_fixed": [[0, 0, 0], [0, 2, 0], [0, 0, 0]],
        "room_state": [[0, 4, 0], [0, 5, 0], [0, 0, 0]],  # Push box up
        "boxes_on_target": 0,
        "max_steps": 5,
        "num_boxes": 1,
    },
    "corridor": {
        "dim_room": [5, 3],
        "room_fixed": [[0, 0, 0], [2, 1, 0], [0, 0, 0]],
        "room_state": [[0, 0, 0], [4, 5, 0], [0, 0, 0]],
        "boxes_on_target": 0,
        "max_steps": 10,
        "num_boxes": 1,
    },
        ...
        for i in range(3):  # Try to exceed limit
            step_resp = await client.post(
                ...
                json={
                    "env_id": env_id,
                    "action": {
                        "tool_calls": [{"tool": "interact", "args": {"action": 1}}]
                    },
                },
            )
    ...
    async def test_invalid_action_handling(self, client):
        ...
        step_resp = await client.post(
            ...
            json={
                "env_id": env_id,
                "action": {
                    "tool_calls": [{"tool": "interact", "args": {"action": 99}}]
                },
            },
        )
    ...
    async def test_concurrent_environments(self, client):
        ...
        for name, puzzle in env_configs:
            resp = await client.post(
                "/env/Sokoban/initialize", json={"initial_state": puzzle}
            )
