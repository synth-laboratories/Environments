...
from examples.sokoban.environment import SokobanEnvironment, SokobanPublicState, SokobanPrivateState
from examples.sokoban.engine import GRID_LOOKUP, _grid_to_text, ACTION_STRING_TO_INT, INT_TO_ACTION_STRING
...
class HistoryObservationCallable(GetObservationCallable):
    ...
    async def get_observation(
        ...
        if pub is None or priv is None:
            ...
            return {"error": "Missing public or private state in get_observation", "history_boards": list(self._hist)} # type: ignore[return-value]
...
class SokobanInteractArgs(BaseModel):
    actions_list: List[str] = Field(description='\'\'\'A sequence of actions to execute in the environment (e.g., ["move up", "push left"])\'\'\'')
...
class Move(EnvToolCall):
    def __init__(self, action: int): self.action = action
        ...
        try:
            if hasattr(response_obj, 'tool_calls') and response_obj.tool_calls:
                ...
                try:
                    ...
                    if isinstance(potential_tool_call_json, dict) and "tool_calls" in potential_tool_call_json:
                        tool_calls = potential_tool_call_json["tool_calls"]
                    elif isinstance(potential_tool_call_json, list) and len(potential_tool_call_json) > 0 and potential_tool_call_json[0].get("type") == "function":
                        tool_calls = potential_tool_call_json 
                    else:
                        self.history.append({
                            "type": "tool_call", "tool_name": "sokoban_interact", 
                            "tool_arguments": {"actions_list": ["move down"], "reasoning": response_obj[:200]}
                        })
                        return ACTION_STRING_TO_INT["move down"]
                except json.JSONDecodeError:
                    self.history.append({
                        "type": "tool_call", "tool_name": "sokoban_interact", 
                        "tool_arguments": {"actions_list": ["move down"], "reasoning": "LLM failed to call tool (JSON decode error), fallback."}
                    })
                    return ACTION_STRING_TO_INT["move down"]
            ...
            if not tool_calls:
                self.history.append({
                    "type": "tool_call", "tool_name": "sokoban_interact", 
                    "tool_arguments": {"actions_list": ["move down"], "reasoning": "LLM failed to provide tool_calls, fallback."}
                })
                return ACTION_STRING_TO_INT["move down"]

            if len(tool_calls) == 0:
                self.history.append({"type": "error", "content": "LLM returned empty tool_calls list."})
            ...
            elif isinstance(tool_call_data, dict) and 'function' in tool_call_data and isinstance(tool_call_data['function'], dict):
            ...
            if not tool_args_str:
                 self.history.append({"type": "error", "content": f"Missing arguments for tool {tool_name}"})
                 return ACTION_STRING_TO_INT["no operation"]
                 
            tool_arguments = json.loads(tool_args_str)


            self.history.append({"type": "tool_call", "tool_name": tool_name, "tool_arguments": tool_arguments})
            ...
            if tool_name == "sokoban_interact":
                ...
                if self.last_obs_dict:
                    ...
                    elif isinstance(raw_boxes_on_target, str) and raw_boxes_on_target.isdigit():
                        boxes_on_target = int(raw_boxes_on_target)

                    is_solved_state = (self.num_total_boxes > 0 and boxes_on_target == self.num_total_boxes)

                    if terminated_by_env or is_solved_state:
                        return -1 
                    else:
                        rejection_reason = (f"Terminate rejected by guard – puzzle not solved. "
            ...
            self.history.append({"type": "error", "content": f"Error processing LLM response: {str(e)}"})
            return ACTION_STRING_TO_INT["no operation"]
...
async def test_react_agent_sokoban(tmp_path: Path):
    ...
    async def run_episode():
        ...
        current_input_to_agent = format_obs_for_llm_from_states(obs_payload["public"], obs_payload["private"])
        ...
        for turn in range(agent.max_turns):
            ...
            current_input_to_agent = format_obs_for_llm_from_states(obs_payload_next["public"], obs_payload_next["private"])
...
async def eval_react_sokoban() -> None:
    ...
    current_model_name_for_eval = "gpt-4.1-nano" # As per user's latest change for this function's scope
    ...
    _temp_llm_for_names = LM(model_name=current_model_name_for_eval, formatting_model_name=current_model_name_for_eval, temperature=0.0)
    ...
    async def run_episode(inst) -> bool:
        ...
        llm_for_episode = LM(model_name=current_model_name_for_eval, formatting_model_name=current_model_name_for_eval, temperature=0.0)
        agent = ReActAgent(llm_for_episode)

        obs = await env.initialize()
        agent.last_obs_dict = {"terminated": obs["private"].terminated, "boxes_on_target": obs["public"].boxes_on_target}
    ...
    async def make_instances(label: str, target_len: int, n: int = 3):
        ...
        while len(instances) < n:
            room_structure, room_state, _, _ = generate_room(dim=(5, 5), initial_seed=seed,
                                                             num_boxes=1, search_depth=max(10, target_len + 2))
            ...
            if len(path) == target_len:
                inst = SokobanTaskInstance(
                    ...
                    impetus=Impetus(instructions="Solve"),
                    intent=Intent(rubric={}, gold_trajectories=None, gold_state_diff={}),
                    metadata=SokobanTaskInstanceMetadata(label, 1, (5, 5), 20, len(path), seed, f"len={target_len}"),
                    is_reproducible=True,
