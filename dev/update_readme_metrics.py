...
def get_test_coverage(include_coverage: bool = False) -> Tuple[float, int, int]:
    ...
    coverage_msg = f" ({coverage_pct:.1f}% coverage)" if include_coverage else " (no coverage)"
...
def get_type_check_results() -> Dict[str, Tuple[int, int]]:
    ...
    file_lines = [line for line in stdout.split('\n') if line.strip().startswith('src/synth_env') and 'examples' not in line and ':' in line]
    ...
    if core_files == 0:
        ...
        for line in all_lines:
            if 'src/synth_env' in line and '.py' in line and 'examples' not in line:
                parts = line.split()
                for part in parts:
                    if 'src/synth_env' in part and '.py' in part and 'examples' not in part:
                        file_mentions.add(part.split(':')[0])
        core_files = len(file_mentions) or 1
    
    results['core'] = (core_errors, core_files)
    
    print(f"✅ Type checking: Examples {examples_errors} errors in {examples_files} files, Core {core_errors} errors in {core_files} files")
    return results

def get_pypi_version(package_name: str = "synth-env") -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            stable_versions = [v for v in versions if not any(x in v.lower() for x in ['a', 'b', 'rc', 'dev', 'alpha', 'beta'])]
            dev_versions = [v for v in versions if any(x in v.lower() for x in ['a', 'b', 'rc', 'dev', 'alpha', 'beta'])]
            
            # Get latest of each
            latest_stable = max(stable_versions, key=lambda x: [int(n) for n in x.split('.') if n.isdigit()]) if stable_versions else None
...
def update_readme_badges(readme_path: Path, metrics: Dict) -> None:
    ...
    test_pct = (metrics['passed_tests'] / metrics['total_tests'] * 100) if metrics['total_tests'] > 0 else 0
    test_status = "passing" if metrics['passed_tests'] == metrics['total_tests'] else "failing"
    ...
    examples_pct = max(0, (examples_files - examples_errors) / examples_files * 100) if examples_files > 0 else 100
    core_pct = max(0, (core_files - core_errors) / core_files * 100) if core_files > 0 else 100
    
    examples_color = "red" if examples_pct < 70 else "yellow" if examples_pct < 90 else "green"
...
def main():
    ...
    parser.add_argument("--coverage", action="store_true", help="Include test coverage analysis (slower)")
    ...
    if 'type_check' in metrics:
        examples_errors, examples_files = metrics['type_check']['examples']
        core_errors, core_files = metrics['type_check']['core']
        print(f"   Type Check Examples: {examples_errors} errors in {examples_files} files")
