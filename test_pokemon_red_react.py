...
from src.examples.red.environment import PokemonRedEnvironment, PokemonRedPublicState, PokemonRedPrivateState
from src.examples.red.taskset import PokemonRedTaskInstance
from src.examples.red.agent_demos.test_synth_react import ReActAgent, PokemonRedHistoryObservationCallable, PressButtonCall
...
async def test_pokemon_red_react_20_steps():
    ...
    hist_cb = PokemonRedHistoryObservationCallable(max_history=3)  # Keep more history for inspection
    ...
    for step in range(20):
        ...
        if agent.history and agent.history[-1]["type"] == "tool_call":
            reasoning = agent.history[-1]["tool_arguments"].get("reasoning", "No reasoning provided")
    ...
    print(f"\n🏆 Reward Summary:")
    print(f"  • Total reward: {total_reward:.3f}")
    print(f"  • Average step reward: {(total_reward / len(step_rewards)):.3f}" if step_rewards else "N/A")
    ...
    for i, entry in enumerate(agent.history[-3:]):
        print(f"  {i+1}. {entry['type']}: {str(entry.get('content', entry.get('tool_arguments', '')))[:100]}...")
