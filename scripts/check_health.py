...
def count_python_lines() -> int:
    ...
    exit_code, stdout, stderr = run_command("find . -name '*.py' -not -path './.*' -not -path './temp/*' -not -path './htmlcov/*' -not -path './build/*' -not -path './dist/*' | xargs wc -l | tail -1")
...
def check_ty_violations() -> tuple[int, dict[str, int]]:
    ...
    exit_code, stdout, stderr = run_command("uvx ty check --output-format concise --exit-zero")
    ...
    if violations_per_file:
        ...
        sorted_files = sorted(violations_per_file.items(), key=lambda x: x[1], reverse=True)
...
def check_ruff_format() -> tuple[int, list[str]]:
    ...
    files_needing_format = [line.strip() for line in stdout.strip().split('\n') if line.strip()]
...
def check_ruff_lint() -> int:
    ...
    lint_issues = len([line for line in stdout.strip().split('\n') if line.strip() and not line.startswith('Found')])
