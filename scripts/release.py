...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Release script: increment version, build, and publish")
    ...
    parser.add_argument("--dev", action="store_true", help="Increment dev version (default)")
    ...
    # Confirm the release
    confirm = input(f"\n❓ Release version {new_version} to {target}? (y/N): ").strip().lower()
    if confirm != 'y':
