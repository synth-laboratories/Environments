...
import json, sqlite3, gzip, pickle, logging
...
class FilesystemSnapshotStore:
    ...
    def write(self, blob: Union[bytes, Dict[str, Any]]) -> str:
        ...
        try:
            if isinstance(blob, dict):
                ...
                compressed_blob = (
                    blob if blob[:2] == b"\x1f\x8b" else gzip.compress(blob)
                )
            else:
                raise TypeError(
                    f"Unsupported blob type for snapshot store: {type(blob)}"
                )
    ...
    def read(self, key: str) -> Optional[bytes]:
        ...
        try:
            ...
            log.error(
                f"Failed to read snapshot {key} from {filepath}: {e}", exc_info=True
            )
...
class TrajectoryTreeStore:
    ...
    def add_root(
        self, snapshot_blob: bytes, *, info: Dict[str, Any] | None = None
    ) -> str:
    ...
    def reconstruct_actions(self, snap_id: str) -> Tuple[Any, ...]:
        ...
        for child, parent in zip(
            self.path_to_root(snap_id)[:-1], self.path_to_root(snap_id)[1:]
        ):
    ...
    def prune_subtree(self, root_id: str) -> None:
        ...
        if self.db_path:
            with sqlite3.connect(self.db_path) as conn:
                conn.executemany(
                    "DELETE FROM nodes WHERE snap_id = ?;", ((n,) for n in doomed)
                )
                conn.executemany(
