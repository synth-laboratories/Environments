"""
Full Enchilada - Crafter Evaluation with Traces, Rewards, and Viewer
...
class FullEnchiladaEvalFramework(CrafterEvalFramework):
    ...
    async def run_single_trajectory_with_trace(
        ...
        try:
            while turn_count < max_turns:
                ...
                # Create AgentComputeStep
                agent_compute_step = AgentComputeStep(
                    event_order=0,
                    compute_began=agent_compute_began,
                    compute_ended=agent_compute_ended,
                    compute_input=[MessageInputs(messages=[
                        {"role": "system", "content": "You are playing Crafter."},
...
async def run_full_enchilada_eval(
    model_names: List[str],
    difficulties: List[str] = ["easy", "hard"], 
    ...
    """Run full enchilada evaluation with traces and viewer."""
    
    framework = FullEnchiladaEvalFramework(capture_images=capture_images, output_dir=output_dir)
