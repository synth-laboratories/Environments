...
ACHIEVEMENT_CATEGORIES = {
    "easy": [
        "collect_wood", "collect_stone", "collect_sapling", "place_stone", 
        "place_table", "wake_up", "eat_plant", "collect_drink"
    ],
    "medium": [
...
class CrafterEvalFramework:
    ...
    def _compute_aggregate_metrics(self, model_name: str, difficulty: str, trajectories: List[TrajectoryResult]) -> AggregateResults:
        ...
        if num_trajectories == 0:
            return AggregateResults(
                ...
                termination_breakdown={}, avg_final_health=None, avg_final_food=None, avg_final_drink=None
            )
    ...
    def _generate_multi_action_analysis(self, aggregate_results: List[AggregateResults]) -> Dict[str, pd.DataFrame]:
        ...
        for agg in aggregate_results:
            summary_data.append({
                ...
                "Most Common": max(agg.actions_per_turn_distribution.items(), key=lambda x: x[1])[0] if agg.actions_per_turn_distribution else 0,
    ...
    def _generate_trajectory_breakdown(self) -> pd.DataFrame:
        ...
        for traj in self.trajectory_results:
            ...
            easy_achievements = len([a for a in traj.achievements_unlocked if a in ACHIEVEMENT_CATEGORIES["easy"]])
            medium_achievements = len([a for a in traj.achievements_unlocked if a in ACHIEVEMENT_CATEGORIES["medium"]])
            hard_achievements = len([a for a in traj.achievements_unlocked if a in ACHIEVEMENT_CATEGORIES["hard"]])
    ...
    def _generate_model_comparison_tables(self, aggregate_results: List[AggregateResults]) -> Dict[str, Any]:
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        sample_cols = ["Model", "Difficulty", "Success", "Steps", "Total Achievements", "Termination"]
        ...
        for comparison in sota_comparison["balrog_nearby_comparisons"]:
            ...
            if "BALROG SOTA Reference" in comp_clean.columns:
                comp_clean = comp_clean.drop(columns=["BALROG SOTA Reference"])  # Too long for display
