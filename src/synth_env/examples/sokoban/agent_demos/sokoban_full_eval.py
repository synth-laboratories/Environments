...
from synth_env.examples.sokoban.engine_helpers.room_utils import generate_room, get_shortest_action_path
...
class SokobanEvalFramework:
    ...
    async def run_single_trajectory_with_trace(
        ...
        instance = SokobanTaskInstance(
            id=uuid.uuid4(),
            impetus=Impetus(instructions="Solve this Sokoban puzzle by pushing all boxes onto targets."),
        ...
        try:
            while turn_count < max_turns:
                ...
                turn_data["action_name"] = list(ACTION_STRING_TO_INT.keys())[list(ACTION_STRING_TO_INT.values()).index(action_int)] if action_int in ACTION_STRING_TO_INT.values() else f"unknown_{action_int}"
                            ...
                            "environment_compute_steps": [{
                                "compute_output": [{
                                    "outputs": {
                                        ...
                                        "action": -1,
                                        "action_name": "final_state",
                                        "player_position": final_turn_data["player_position"],
                                        "boxes_on_target": final_turn_data["boxes_on_target"],
                        ...
                        "events": [{
                            ...
                            "agent_compute_step": {
                                ...
                                "compute_input": [
                                    {
                                        "messages": [
                                            {"role": "system", "content": "You are playing Sokoban. Push all boxes onto targets."},
                                            {"role": "user", "content": f"Turn {i+1}: {turn_data['room_text']}"}
                                        ]
                                    }
                                ],
                                "compute_output": [
                                                ...
                                                "tool_calls": [{
                                                    ...
                                                    "function": {
                                                        ...
                                                        "arguments": json.dumps({
                                                            "actions_list": [turn_data.get('action_name', 'initial')],
                                ...
                                "compute_input": [
                                    {
                                        ...
                                        "action_name": turn_data.get("action_name", "initial")
                                    }
                                ],
                                "compute_output": [{
                                    "outputs": {
        ...
        # Save trace file
        eval_dir = eval_dir or Path(f"src/evals/sokoban/run_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        for model_name in model_names:
            for difficulty in difficulties:
                ...
                for traj_idx in range(num_trajectories_per_condition):
                    ...
                    print(f"   Trajectory {traj_idx + 1}/{num_trajectories_per_condition} (seed={seed})")
        ...
        for model_name in model_names:
            for difficulty in difficulties:
                ...
                if condition_results:
                    ...
                    avg_reward = sum(r.final_reward for r in condition_results) / len(condition_results)
                    avg_steps = sum(r.num_steps for r in condition_results) / len(condition_results)
                    avg_boxes = sum(r.boxes_solved for r in condition_results) / len(condition_results)
