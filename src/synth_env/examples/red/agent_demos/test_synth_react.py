...
from synth_env.examples.red.environment import (
...
from synth_env.examples.red.engine_helpers.reward_library.pallet_town_rewards import (
...
from synth_env.examples.red.engine_helpers.reward_library.exploration_rewards import (
    NewAreaDiscoveryReward,
    BuildingEntryReward,
)
from synth_env.examples.red.engine_helpers.reward_library.novelty_rewards import (
    FirstBattleReward,
...
from synth_env.examples.red.taskset import PokemonRedTaskInstance
from synth_env.tasks.core import Impetus, Intent, TaskInstanceMetadata
from synth_env.environment.tools import EnvToolCall

# Import screen analysis functions
from synth_env.examples.red.engine_helpers.screen_analysis import (
        ...
        for reward_component in self.rewards:
            try:
                ...
                if reward > 0:
                    ...
                    print(
                        f"[REWARD] {reward_component.__class__.__name__}: +{reward:.1f}"
                    )
            except Exception as e:
...
def format_obs_for_llm_from_states(
    ...
    if screen_analysis:
        ...
        if mode == "state_and_ascii":
            ...
            screen_description = (
                f"SCREEN TYPE: {screen_analysis.get('screen_type', 'UNKNOWN')}\n"
            )
            ...
            if "colors" in screen_analysis:
                colors_text = "DOMINANT COLORS: " + ", ".join(
                    [
                        f"{color}({pct}%)"
                        for color, pct in screen_analysis["colors"].items()
                    ]
                )
...
class PokemonRedHistoryObservationCallable(GetObservationCallable):
    ...
    async def get_observation(
        ...
        if pub is None or priv is None:
            raise RuntimeError(
                "Missing public or private state in get_observation - HARD FAIL"
            )
        ...
        current_state_hash = hash(
            (pub.player_x, pub.player_y, pub.map_id, pub.step_count)
        )
        ...
        if self.mode == "state_and_ascii":
            ...
            formatted_obs = format_obs_for_llm_from_states(
                pub, priv, screen_analysis, self.mode
            )
        else:
        ...
        if additional_reward > 0 or self.reward_manager.total_reward_earned > 0:
            ...
            reward_info += (
                f"Total Rewards Earned: {self.reward_manager.total_reward_earned:.1f}\n"
            )
            ...
            if self.reward_manager.reward_history:
                ...
                for achievement in self.reward_manager.reward_history[-3:]:
                    for component in achievement["components"]:
                        reward_info += (
                            f"• {component['component']}: +{component['reward']:.1f}\n"
                        )
    ...
    def validate_buttons(cls, v):
        ...
        for button in v:
            if button.upper() not in valid_buttons:
                raise ValueError(
                    f"Invalid button: {button}. Valid buttons: {valid_buttons}"
                )
...
class ReActAgent:
    ...
    def _get_recent_reasoning_traces(self, k: int = 5) -> str:
        ...
        for tool_call in tool_calls[-k:]:
            if (
                "tool_arguments" in tool_call
                and "reasoning" in tool_call["tool_arguments"]
            ):
                step_num = len(
                    [
                        e
    ...
    async def decide(
        ...
        if current_raw_obs and isinstance(
            current_raw_obs.get("public"), PokemonRedPublicState
        ):
        ...
        if mode == "state_and_screen":
            try:
                ...
                if (
                    ...
                    print(
                        f"[AGENT_DEBUG] Got screen buffer with shape: {screen_buffer.shape}"
                    )
                    ...
                    if (
                        len(screen_array.shape) == 3 and screen_array.shape[2] == 4
                    ):  # RGBA
                        # Convert RGBA to RGB by dropping alpha channel
                        image = Image.fromarray(screen_array[:, :, :3], mode="RGB")
                    else:
                        raise ValueError(
                            f"Unsupported screen array shape: {screen_array.shape}"
                        )
        ...
        if mode == "state_and_ascii":
            system_message += "• Visual Screen Analysis: ASCII representation and entity detection\n\n"
        else:
            system_message += "• Visual Screen Analysis: ASCII representation and actual screen images\n\n"
        ...
        try:
            ...
            if not tool_args_str:
                print(
                    f"[AGENT_DEBUG] Missing arguments for tool {tool_name}, falling back to A"
                )
                self.history.append(
                    {
                        "type": "error",
            ...
            if tool_name == "pokemon_red_interact":
                ...
                invalid_buttons = [
                    button for button in buttons if button not in self.valid_buttons
                ]
                if invalid_buttons:
                    print(
                        f"[AGENT_DEBUG] Invalid buttons: {invalid_buttons}, falling back to A"
                    )
                    self.history.append(
                        {
                            "type": "error",
                ...
                print(
                    f"[AGENT_DEBUG] Unknown tool_name: {tool_name}, falling back to A"
                )
                self.history.append(
...
async def test_react_agent_pokemon_red(
    ...
    async def run_episode():
        ...
        for turn in range(agent.max_turns):
            buttons = await agent.decide(
                current_formatted_obs, raw_obs_for_agent_decision, mode
            )
    ...
    async def run_episode_eval(
        ...
        for turn_idx in range(agent.max_turns):
            ...
            if obs_payload.get("screen_buffer") is not None:
                ...
                min_screen_unchanged = (
                    12  # Increased - Pokemon Red often has static screens
                )
...
if __name__ == "__main__":
    ...
    asyncio.run(
        eval_react_pokemon_red(
            model_name="gpt-4.1-mini", max_turns=10, mode="state_and_screen"
        )
    )
