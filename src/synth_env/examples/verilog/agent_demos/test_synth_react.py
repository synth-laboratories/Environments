...
class CompileArgs(BaseModel):
    sources: Optional[List[str]] = Field(
        None, description="List of source files to compile"
    )
...
class Compile(EnvToolCall):
    def __init__(
        self, sources: Optional[List[str]] = None, testbench: Optional[str] = None
    ):
        super().__init__(
            tool="compile", args={"sources": sources, "testbench": testbench}
        )
        ...
        try:
            ...
            if hasattr(tool_call, "function"):
                ...
                return {
                    "action": "terminate",
                    "args": {
                        "reason": f"Unexpected tool call format: {type(tool_call)}"
                    },
                }
...
async def run_verilog_episode(
    ...
    try:
        ...
        for turn in range(agent.max_turns):
            ...
            if decision["action"] == "terminate":
                ...
                agent.history.append(
                    {"type": "tool_response", "content": f"Terminated: {reason}"}
                )
                if debug:
            ...
            if action_name == "write_file":
                ...
                tool_call = Compile(
                    action_args.get("sources"), action_args.get("testbench")
                )
            ...
            if obs.get("terminated", False):
                ...
                if debug:
                    print(
                        f"[DEBUG] Environment terminated. Task completed: {task_completed}"
                    )
        ...
        if debug:
            print(
                f"[DEBUG] Episode ended after {agent.max_turns} turns without completion"
            )
            print(f"[DEBUG] Final observation: {obs}")
...
async def eval_verilog_react(
    ...
    for task_instance in taskset.instances:
        ...
        for i in range(n_instances):
            ...
            success = await run_verilog_episode(
                task_instance, model_name, debug=debug_mode
            )
        ...
        print(
            f"  Task {task_name}: {success_count}/{len(task_results)} ({success_rate:.1%})"
        )
...
async def run_parallel_evaluation(
    models_to_test=["gpt-4.1-nano", "gpt-4.1-mini"], n_instances=3
):
