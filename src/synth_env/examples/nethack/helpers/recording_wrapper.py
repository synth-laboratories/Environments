...
class RecordingNetHackEnvironment(NetHackEnvironment):
    ...
    async def start(self, **kwargs) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        if self.auto_record:
            ...
            if hasattr(self.task_instance, 'metadata') and hasattr(self.task_instance.metadata, 'character_role'):
    ...
    async def process_action(self, tool_calls: list[EnvToolCall]) -> Tuple[Any, float, bool, Dict[str, Any]]:
    ...
    def start_recording(self, character_role: Optional[str] = None, 
