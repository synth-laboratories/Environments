...
from src.synth_env.examples.nethack.helpers.trajectory_recorder import TrajectoryRecorder, TrajectoryFrame
...
class ReplayViewer:
    ...
    def __init__(self, trajectory_path: str):
        ...
        # Load trajectory
        self.recorder, self.metadata, self.frames = TrajectoryRecorder.load_trajectory(str(trajectory_path))
    ...
    def show_frame(self, frame_idx: int):
        ...
        if 0 <= frame_idx < len(self.frames):
            ...
            print(f"Position: ({stats.get('x', 0)}, {stats.get('y', 0)}) | "
    ...
    def show_trajectory_info(self):
        ...
        print(f"Duration: {(self.metadata.end_time - self.metadata.start_time).total_seconds():.1f} seconds")
        print(f"Total steps: {self.metadata.total_steps}")
        print(f"Total reward: {self.metadata.total_reward:.2f}")
...
def main():
    ...
    parser.add_argument("--export", choices=['video', 'stats', 'actions'],
