...
def simplify_observation(observation: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "ascii_map" in observation:
        ...
        simplified["at_stairs"] = any(
            pos == map_info["player_position"] 
            for _, pos in map_info["stairs_positions"]
        )
...
def identify_item_type(description: str) -> str:
    ...
    # Weapons
    if any(word in desc_lower for word in ["sword", "dagger", "spear", "axe", "mace", 
                                            "bow", "arrow", "dart", "knife"]):
        return "weapon"
    
    # Armor
    if any(word in desc_lower for word in ["armor", "mail", "helmet", "boots", "gloves",
                                            "shield", "cloak", "robe"]):
        return "armor"
    
    # Food
    if any(word in desc_lower for word in ["food", "ration", "corpse", "egg", "fruit",
                                            "meat", "candy", "cookie"]):
        return "food"
    ...
    # Tools
    if any(word in desc_lower for word in ["pick", "key", "lamp", "candle", "bag",
                                            "sack", "horn", "whistle", "mirror"]):
        return "tool"
