...
class NLEWrapper:
    ...
    def __init__(self, character_role: str = "mon", observation_keys: Optional[List[str]] = None):
        ...
        if observation_keys is None:
            observation_keys = [
                "glyphs", "chars", "colors", "specials",
                "blstats", "message", "inv_glyphs", "inv_letters",
                "inv_oclasses", "inv_strs", "tty_chars", "tty_colors",
                "tty_cursor"
            ]
        
        # Create NLE environment
        self.env = nle.env.NLE(
            character=self.character_role,
        ...
        if action not in self.action_map:
            ...
            if len(action) == 1 and action.isalpha():
                ...
                for i, act in enumerate(self.env.actions):
                    ...
                    raise ValueError(f"Unknown action: {action}. Valid actions: {list(self.action_map.keys())}")
            else:
                raise ValueError(f"Unknown action: {action}. Valid actions: {list(self.action_map.keys())}")
        else:
    ...
    def _process_observation(self, obs: Dict[str, np.ndarray]) -> Dict[str, Any]:
        ...
        if 'tty_chars' in obs:
            ...
            processed['in_menu'] = any(
                ...
                for keyword in ['pick up', 'drop', 'wear', 'take off', 'what do you want']
            )
