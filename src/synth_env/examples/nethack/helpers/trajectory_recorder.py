    ...
    def _serialize_observation(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in obs.items():
            if isinstance(value, np.ndarray):
                ...
                serialized[key] = [self._serialize_observation(item) if isinstance(item, dict) else item for item in value]
            else:
    ...
    def _deserialize_observation(obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in obs.items():
            if isinstance(value, dict) and value.get('type') == 'ndarray':
                deserialized[key] = np.array(value['data'], dtype=value['dtype']).reshape(value['shape'])
            elif isinstance(value, dict):
                deserialized[key] = TrajectoryFrame._deserialize_observation(value)
            elif isinstance(value, list) and len(value) > 0 and isinstance(value[0], dict):
                deserialized[key] = [TrajectoryFrame._deserialize_observation(item) if isinstance(item, dict) else item for item in value]
            else:
...
class TrajectoryRecorder:
    ...
    def stop_recording(self, final_status: str = "completed", 
    ...
    @classmethod
    def load_trajectory(cls, filepath: str) -> Tuple['TrajectoryRecorder', TrajectoryMetadata, List[TrajectoryFrame]]:
