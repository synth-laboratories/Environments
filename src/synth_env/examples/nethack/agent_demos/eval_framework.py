...
_achievements_path = os.path.join(os.path.dirname(__file__), "..", "achievements.json")
...
ACHIEVEMENT_CATEGORIES = {
    "basic": [
        "first_kill", "first_spell_cast", "first_prayer", "survived_100_turns",
        "reached_dlvl_2", "reached_dlvl_5", "killed_10_monsters"
    ],
    "intermediate": [
        "reached_dlvl_10", "reached_dlvl_20", "killed_50_monsters", "killed_100_monsters",
        "collected_1000_gold", "reached_level_5", "reached_level_10", "reached_minetown"
    ],
    "advanced": [
        "reached_dlvl_30", "reached_castle", "got_quest", "completed_quest",
        "reached_level_20", "collected_10000_gold", "found_artifact", "reached_mines_end"
    ]
}
...
class NetHackEvalFramework:
    ...
    async def run_single_trajectory(
        ...
        instance = NetHackTaskInstance(
            id=uuid.uuid4(),
            impetus=Impetus(instructions=f"Explore the NetHack dungeon on {difficulty} difficulty. Survive as long as possible, kill monsters, collect items, and descend to deeper levels."),
...
"""
                ...
                # Update progress bar
                easy_count = len([a for a in achievements_unlocked if a in ACHIEVEMENT_CATEGORIES["basic"]])
                inter_count = len([a for a in achievements_unlocked if a in ACHIEVEMENT_CATEGORIES["intermediate"]])
                adv_count = len([a for a in achievements_unlocked if a in ACHIEVEMENT_CATEGORIES["advanced"]])
                ...
                for action in actions:
                    ...
                    if private_state:
                        if getattr(private_state, "terminated", False) or getattr(private_state, "truncated", False):
                            termination_reason = "timeout" if getattr(private_state, "truncated", False) else "death"
                            balrog_progress.update(current_depth, current_level, done=True, end_reason=termination_reason)
                            break
    ...
    def _compute_aggregate_metrics(self, model_name: str, difficulty: str, trajectories: List[TrajectoryResult]) -> AggregateResults:
        ...
        if num_trajectories == 0:
            return AggregateResults(
                ...
                termination_breakdown={}, avg_final_depth=None, avg_final_level=None, avg_final_gold=None
            )
    ...
    def _generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "termination_breakdown_table": self._generate_termination_breakdown_table(aggregate_results),
    ...
    def _generate_achievement_percentage_table(self, grouped_results: Dict[str, Dict[str, List[TrajectoryResult]]]) -> pd.DataFrame:
    ...
    def _generate_termination_breakdown_table(self, aggregate_results: List[AggregateResults]) -> pd.DataFrame:
    ...
    def _generate_trajectory_breakdown(self) -> pd.DataFrame:
        ...
        for traj in self.trajectory_results:
            ...
            basic_achievements = len([a for a in traj.achievements_unlocked if a in ACHIEVEMENT_CATEGORIES["basic"]])
            inter_achievements = len([a for a in traj.achievements_unlocked if a in ACHIEVEMENT_CATEGORIES["intermediate"]])
            adv_achievements = len([a for a in traj.achievements_unlocked if a in ACHIEVEMENT_CATEGORIES["advanced"]])
    ...
    def _generate_sota_comparison(self, aggregate_results: List[AggregateResults]) -> Dict[str, pd.DataFrame]:
        ...
        for agg in aggregate_results:
            ...
            if agg.model_name in MODEL_NAME_TO_SOTA:
                ...
                if sota_name in BALROG_SOTA_SCORES["balrog_leaderboard"]:
                    ...
                    percentage_of_balrog_sota_best = (agg.balrog_score_best / balrog_sota_score) * 100
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        sample_cols = ["Model", "Difficulty", "Success", "Steps", "Total Achievements", "BALROG Score", "Termination"]
    ...
    def _generate_achievement_summary_table(self, grouped_results: Dict[str, Dict[str, List[TrajectoryResult]]]) -> pd.DataFrame:
            ...
            for achievement in achievements:
                ...
                for model_name, difficulties in grouped_results.items():
                    for difficulty, trajectories in difficulties.items():
                        ...
                        num_trajectories = len(trajectories)
                        count = sum(1 for traj in trajectories if achievement in traj.achievements_unlocked)
                        percentage = count / num_trajectories if num_trajectories > 0 else 0.0
