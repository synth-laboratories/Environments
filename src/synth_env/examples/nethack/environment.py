...
from .helpers import validate_action, get_action_description, NETHACK_ACTIONS, MENU_ACTIONS
...
class NetHackEnvironment(StatefulEnvironment, ReproducibleEnvironment[NetHackEngine]):
    ...
    def __init__(self, 
        ...
        self.custom_checkpoint_observation_callable = custom_ckpt_obs or NetHackCheckpointObservationCallable()
    ...
    async def step(self, tool_calls: Union[List[EnvToolCall], EnvToolCall, Dict, List[Dict], str]) -> InternalObservation:
    ...
    def validate_tool_calls(self, tool_calls: Union[List[EnvToolCall], EnvToolCall, Dict, List[Dict], str]) -> EnvToolCall:
        """Validate and normalize tool calls."""
        # Handle string input (simple action)
        if isinstance(tool_calls, str):
            return EnvToolCall(
                tool="interact",
                args={"action": tool_calls}
            )
