...
class NetHackPublicState:
    ...
    def diff(self, prev_state: "NetHackPublicState") -> Dict[str, Any]:
        ...
        if self.dungeon_level != prev_state.dungeon_level:
            differences["dungeon_level"] = (prev_state.dungeon_level, self.dungeon_level)
...
class NetHackPrivateState:
    ...
    def diff(self, prev_state: "NetHackPrivateState") -> Dict[str, Any]:
        ...
        if self.depth_reached != prev_state.depth_reached:
            differences["depth_reached"] = (prev_state.depth_reached, self.depth_reached)
...
class NetHackEngine(StatefulEngine, IReproducibleEngine):
    ...
    async def _reset_engine(self, *, seed: int | None = None) -> Tuple[NetHackPrivateState, NetHackPublicState]:
        ...
        self.public_state = NetHackPublicState(
            ...
            message=obs['message'],
            cursor_position=obs.get('cursor', (player_stats['y'], player_stats['x'])),  # Cursor might not be in processed obs
            turn_count=0,
...
# print(f"===== NetHack Engine _step_engine called with action: {action} =====")
        ...
        if action not in self.nle.action_map and action not in ['terminate']:
            ...
            if len(action) == 1 and (action.isalpha() or action.isdigit()):
                ...
                raise ValueError(f"Invalid action: {action}. Valid actions: {list(self.nle.action_map.keys())}")
        ...
        non_turn_actions = ["look", "farlook", "whatis", "identify", "discoveries", 
                           "conduct", "attributes", "help", "version", "history"]
        ...
        if action in non_turn_actions:
            ...
            if hasattr(self, '_consecutive_free_actions'):
                ...
                if self._consecutive_free_actions >= 3:
                    logger.warning(f"Too many consecutive free actions ({self._consecutive_free_actions}), forcing 'wait'")
                    action = "wait"
        ...
        self.private_state.depth_reached = max(self.private_state.depth_reached, player_stats['depth'])
        ...
        self.public_state.cursor_position = obs.get('cursor', (player_stats['y'], player_stats['x']))
        ...
        # Update inventory
        self.public_state.inventory = self._process_inventory(obs['inventory']) if 'inventory' in obs else []
        ...
        if done:
            ...
            if 'end_status' in info and info['end_status'] == 0:  # 0 means death
                self.public_state.message = info.get('death_reason', 'You died!')  # death_reason might not always exist
            else:
        ...
        self.public_state.achievements_unlocked.update(self.public_state.achievements.get_unlocked_achievements())
        ...
        if self.private_state.balrog_reward_last > 0:
            print(f"🏆 BALROG REWARD: +{self.private_state.balrog_reward_last:.3f} (total: {self.private_state.balrog_total_reward:.3f})")
            balrog_score = self.public_state.achievements.balrog_progress.percent
            print(f"   Balrog score: {balrog_score}% (dungeon: {self.public_state.achievements.balrog_progress.dungeon_progression}, exp: {self.public_state.achievements.balrog_progress.experience_progression})")
    ...
    async def _deserialize_engine(cls, snapshot: NetHackEngineSnapshot) -> "NetHackEngine":
        ...
        engine.public_state = NetHackPublicState(
            ...
            cursor_position=(pub_data["cursor_position"][0], pub_data["cursor_position"][1]),
    ...
    def get_current_states_for_observation(self) -> Tuple[NetHackPrivateState, NetHackPublicState]:
...
class NetHackObservationCallable(GetObservationCallable):
    ...
    async def get_observation(self, pub: NetHackPublicState, priv: NetHackPrivateState) -> InternalObservation:
        observation = {
