...
from src.synth_env.examples.nethack.helpers.trajectory_recorder import (
    TrajectoryRecorder, TrajectoryFrame, TrajectoryMetadata
)
...
class TestTrajectoryRecorder(unittest.TestCase):
    ...
    def test_record_step(self):
        ...
        obs = {
            'player_stats': {
                'x': 10, 'y': 20, 'hp': 16, 'max_hp': 16,
                'depth': 1, 'gold': 0, 'experience_level': 1
            },
            'ascii_map': "###\n#@#\n###",
...
######"""
    ...
    def test_create_frame_image(self):
        ...
        obs = {
            'ascii_map': "#####\n#@..#\n#####",
            'player_stats': {
                'x': 1, 'y': 1, 'hp': 10, 'max_hp': 10,
                'experience_level': 1, 'depth': 1, 'gold': 0
            },
            'message': "Test message"
        }
...
class TestTrajectoryFrame(unittest.TestCase):
    ...
    def test_frame_serialization(self):
        ...
        np.testing.assert_array_equal(
            restored_frame.observation['array_data'],
            np.array([1, 2, 3])
        )
        np.testing.assert_array_equal(
            restored_frame.observation['nested']['matrix'],
