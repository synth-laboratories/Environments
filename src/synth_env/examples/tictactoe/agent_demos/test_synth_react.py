...
class TicTacToeReActAgent:
    ...
    async def decide(self, obs: str) -> Dict[str, Any]:
        ...
        if hasattr(response, "tool_calls") and response.tool_calls:
            ...
            return {
                "name": "terminate",
                "parameters": {"reason": "No valid action"}
            }
...
async def eval_react_tictactoe(model_name: str = "gpt-4.1-mini", num_episodes: int = 10) -> List[Dict[str, Any]]:
    ...
    for i, instance in enumerate(taskset.instances[:num_episodes]):
        ...
        # Print result
        print(f"  Result: {result['winner']}, Moves: {len(result['moves'])}, Reward: {result['final_reward']}")
