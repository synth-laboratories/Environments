...
class TicTacToeEngine(StatefulEngine, IReproducibleEngine):
    def __init__(self, task_instance: TaskInstance):
        ...
        if hasattr(task_instance, "metadata") and hasattr(task_instance.metadata, "starting_player"):
    ...
    async def _reset_engine(self, *, seed: int | None = None) -> Tuple[TicTacToePrivateState, TicTacToePublicState]:
        self.board = np.zeros(9, dtype=int)
        self.current_player = "X"
        self.last_move = None
        self.winner = None
        ...
        # Apply any pre-moves from task instance metadata
        if hasattr(self.task_instance, "metadata") and hasattr(self.task_instance.metadata, "opening_moves"):
    ...
    def get_current_states_for_observation(self) -> Tuple[TicTacToePrivateState, TicTacToePublicState]:
        public_state = TicTacToePublicState(
            board=self.board.copy(),
            current_player=self.current_player,
            last_move=self.last_move,
...
class SynthTicTacToeObservationCallable(GetObservationCallable):
    async def get_observation(self, pub: TicTacToePublicState, priv: TicTacToePrivateState) -> InternalObservation:
