...
from synth_env.examples.minigrid.taskset import (
...
async def test_default_task():
    ...
    assert task.intent.rubric["goal"] == "Successfully reach the goal tile in the MiniGrid-Empty-5x5-v0 environment."
...
async def test_create_taskset():
    ...
    taskset = await create_minigrid_taskset(
        num_tasks_per_difficulty={"easy": 5, "medium": 3, "hard": 2},
        seed=42
    )
    ...
    for instance in taskset.instances:
        ...
        assert instance.metadata.env_name in [env[0] for envs in ENVIRONMENTS.values() for env in envs]
...
async def test_task_metadata():
    ...
    taskset = await create_minigrid_taskset(
        num_tasks_per_difficulty={"easy": 2, "medium": 2, "hard": 2},
        seed=123
    )
...
async def test_task_instructions():
    ...
    taskset = await create_minigrid_taskset(
        num_tasks_per_difficulty={"medium": 5},
        seed=456
    )
...
async def test_reproducibility():
    ...
    taskset1 = await create_minigrid_taskset(
        num_tasks_per_difficulty={"easy": 3, "medium": 3},
        seed=seed
    )
