...
async def eval_minigrid_react(
    ...
    taskset = await create_minigrid_taskset(
        num_tasks_per_difficulty={difficulty: num_tasks},
        seed=42
    )
    ...
    for i, task in enumerate(taskset.instances[:num_tasks]):
        ...
        debug_log_file = os.path.join(debug_dir, f"task_{i+1}_{model_name.replace('.', '_')}_debug.json")
...
async def main():
    ...
    for model, summary in all_results.items():
        print(f"{model:<20} {summary['success_rate']:.1%}{'':10} "
