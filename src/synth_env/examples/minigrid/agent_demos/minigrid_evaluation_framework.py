...
def get_pure_success_scores(aggregate_results: List[MiniGridAggregateResults]) -> Dict[str, float]:
    """
...
def get_success_rate(report: Dict[str, Any], model_name: str, difficulty: str = None) -> float:
    ...
    if difficulty:
        ...
        matching_scores = [score for key, score in success_scores.items() if key.startswith(model_name)]
    ...
    def _compute_aggregate_metrics(self, model_name: str, difficulty: str, 
        ...
        if num_trajectories == 0:
            return MiniGridAggregateResults(
                ...
                avg_achievements_per_trajectory=0.0, achievement_unlock_rates={},
                composite_score_avg=0.0, composite_score_best=0.0,
                navigation_score_avg=0.0, navigation_score_best=0.0,
        ...
        composite_scores = [minigrid_composite_score(len(traj.achievements_unlocked), 
                                                   1.0 if traj.success else 0.0, 
                                                   traj.efficiency_ratio,
                                                   traj.exploration_coverage) for traj in trajectories]
        composite_score_avg = sum(composite_scores) / len(composite_scores) if composite_scores else 0.0
        composite_score_best = max(composite_scores) if composite_scores else 0.0
        
        # Navigation scores
        navigation_scores = [minigrid_navigation_score(1.0 if traj.success else 0.0,
                                                     traj.efficiency_ratio,
                                                     traj.wall_collision_count / max(traj.total_turns, 1)) 
                           for traj in trajectories]
        navigation_score_avg = sum(navigation_scores) / len(navigation_scores) if navigation_scores else 0.0
