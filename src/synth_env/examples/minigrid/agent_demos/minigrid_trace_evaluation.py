...
async def run_simple_minigrid_eval(
    ...
    for episode in range(num_episodes):
                ...
                "events": [{
                    "environment_compute_steps": [{
                        "compute_output": [{
                            "outputs": {
                                "observation": {
                                    "mission": getattr(env.unwrapped, 'mission', "Reach the goal"),
                                    "image_base64": image_to_base64(obs if isinstance(obs, np.ndarray) else obs['image'])
                                },
                                "action": action,
                                "reward": float(reward),
        ...
        trace_data["dataset"]["reward_signals"].append({
...
if __name__ == "__main__":
    # Run evaluation
    asyncio.run(run_simple_minigrid_eval(
        env_name="MiniGrid-Empty-6x6-v0",
        num_episodes=3,
        max_steps=30
    ))