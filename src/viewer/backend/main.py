...
async def evaluations(env: Optional[str] = Query(None, description="Filter by environment")):
                    ...
                    if summary_file.exists():
                        with open(summary_file, 'r') as f:
                            ...
                            evaluations.append(Evaluation(
                                ...
                                models_evaluated=",".join(summary.get("models", [])),
...
async def traces(env_name: str, run_id: str):
    ...
    try:
        ...
        if not traces_dir.exists():
            raise HTTPException(status_code=404, detail=f"Traces not found for {env_name}/{run_id}")
