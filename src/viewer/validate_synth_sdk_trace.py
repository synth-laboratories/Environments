...
from synth_sdk.tracing.abstractions import (
    SystemTrace, Event, AgentComputeStep, EnvironmentComputeStep,
        ...
        for partition_idx, partition in enumerate(partitions):
            ...
            for event_idx, event in enumerate(events):
                ...
                if 'agent_compute_step' not in event:
                    report["warnings"].append(f"{event_path}: Missing 'agent_compute_step'")
                elif event['agent_compute_step'] is None:
                    report["warnings"].append(f"{event_path}: 'agent_compute_step' is None")
                else:
                    ...
                    if 'compute_input' not in agent_step:
                        report["errors"].append(f"{event_path}: Agent step missing 'compute_input'")
                        report["is_valid"] = False
                    elif not agent_step['compute_input']:
                        report["warnings"].append(f"{event_path}: Agent step has empty 'compute_input'")
                    else:
                        # Check for messages in compute_input
                        has_messages = False
                        for input_item in agent_step['compute_input']:
                            if isinstance(input_item, dict) and 'messages' in input_item:
                                has_messages = True
                                ...
                                if messages:
                                    ...
                                    for msg in messages:
                                        if isinstance(msg, dict) and 'tool_calls' in msg:
                                            report["stats"]["events_with_tool_calls"] += 1
                                            break
                                break
                        
                        if not has_messages:
                            report["warnings"].append(f"{event_path}: Agent step has no messages in compute_input")
                    
                    if 'compute_output' not in agent_step:
                        report["errors"].append(f"{event_path}: Agent step missing 'compute_output'")
                        report["is_valid"] = False
                    elif not agent_step['compute_output']:
                        report["warnings"].append(f"{event_path}: Agent step has empty 'compute_output'")
                    
                    if 'model_name' not in agent_step:
                        report["warnings"].append(f"{event_path}: Agent step missing 'model_name'")
                
                # Check for environment compute steps
                if 'environment_compute_steps' not in event:
                    report["warnings"].append(f"{event_path}: Missing 'environment_compute_steps'")
                elif event['environment_compute_steps']:
                    report["stats"]["events_with_env_steps"] += 1
                    
                    # Validate environment steps
                    for env_step_idx, env_step in enumerate(event['environment_compute_steps']):
                        if 'compute_output' not in env_step:
                            report["errors"].append(f"{event_path}, EnvStep {env_step_idx}: Missing 'compute_output'")
                            report["is_valid"] = False
...
def assert_valid_synth_sdk_trace(trace_data: Dict[str, Any], trace_name: str = ""):
    ...
    if not report["is_valid"]:
        error_msg = f"Invalid Synth SDK trace{' (' + trace_name + ')' if trace_name else ''}:\n"
        for error in report["errors"]:
            error_msg += f"  - {error}\n"
    ...
    print(f"✅ Synth SDK trace{' (' + trace_name + ')' if trace_name else ''} validation passed!")
...
def assert_trace_has_agent_reasoning(trace_data: Dict[str, Any], trace_name: str = ""):
    ...
    if report["stats"]["events_with_agent_steps"] == 0:
        raise AssertionError(f"Trace{' (' + trace_name + ')' if trace_name else ''} has no agent compute steps!")
    
    if report["stats"]["events_with_messages"] == 0:
        raise AssertionError(f"Trace{' (' + trace_name + ')' if trace_name else ''} has no messages in agent compute steps!")
    
    # Check that we have a reasonable ratio of agent steps to total events
    agent_ratio = report["stats"]["events_with_agent_steps"] / report["stats"]["total_events"]
    if agent_ratio < 0.5:
    ...
    print(f"✅ Trace{' (' + trace_name + ')' if trace_name else ''} has proper agent reasoning!")


if __name__ == "__main__":
    # Test validation on existing traces
    sys.path.append('/Users/joshuapurtell/Documents/GitHub/Environments/src')
    from streamlit_app import get_db_connection, load_environments, load_evaluations, load_trajectories, load_trace_data
    
    conn = get_db_connection()
    
    # Test Sokoban traces
    print("Testing Sokoban traces...")
    sokoban_env = [env for env in load_environments(conn) if env['name'] == 'sokoban'][0]
