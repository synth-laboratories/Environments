...
def get_db_connection():
    ...
    if db_path_env:
        ...
        if not db_path.exists():
            st.error(f"Database not found at {db_path} (from TRACE_DB environment variable)")
            return None
    else:
        # Try multiple possible locations for the database
        possible_paths = [
            Path(__file__).parent.parent.parent / "synth_eval.duckdb",  # Environments/synth_eval.duckdb
        ...
        if not db_path:
            ...
            st.info("Please ensure TRACE_DB environment variable is set or synth_eval.duckdb exists in one of the expected locations")
            return None
    ...
    try:
        ...
        try:
            conn.execute("ALTER TABLE evaluations ADD COLUMN archived BOOLEAN DEFAULT FALSE")
        except:
...
def load_environments(conn):
    ...
    try:
        result = conn.execute("SELECT id, name, display_name FROM environments ORDER BY name").fetchall()
        return [{"id": row[0], "name": row[1], "display_name": row[2]} for row in result]
    except Exception as e:
...
def archive_evaluation(conn, evaluation_id, archived=True):
    ...
    try:
        conn.execute("UPDATE evaluations SET archived = ? WHERE id = ?", [archived, evaluation_id])
        return True
    except Exception as e:
        ...
        for partition in partitions:
            ...
            for event in events:
                if 'agent_compute_step' in event:
                    ...
                    elif 'compute_input' in agent_step and len(agent_step['compute_input']) > 0:
    ...
    try:
        ...
        if eval_result:
            ...
            possible_paths = [
                ...
                Path(f"../../../src/evals/{env_name}/{run_id}/traces/{trace_id}.json"),
                # Legacy structure
                Path(f"../../synth_env/examples/{env_name}/agent_demos/src/evals/{env_name}/{run_id}/traces/{trace_id}.json"),
                Path(f"../synth_env/examples/{env_name}/agent_demos/src/evals/{env_name}/{run_id}/traces/{trace_id}.json"),
                Path(f"../../../synth_env/examples/{env_name}/agent_demos/src/evals/{env_name}/{run_id}/traces/{trace_id}.json"),
...
def discover_evaluation_runs():
    ...
    search_paths = [
        ...
        Path("../synth_env/examples/crafter_classic/agent_demos/src/evals/crafter/run_*"),
        Path("../../synth_env/examples/crafter_classic/agent_demos/src/evals/crafter/run_*"),
        Path("../../../synth_env/examples/crafter_classic/agent_demos/src/evals/crafter/run_*"),
    ...
    for search_path in search_paths:
        for run_dir in glob.glob(str(search_path)):
            ...
            if run_path.is_dir():
                ...
                if (summary_file.exists() and traces_dir.exists()) or (traces_dir.exists() and len(list(traces_dir.glob("*.json"))) > 0):
...
def import_evaluation_run_streamlit(run_path: Path, conn):
    ...
    try:
        ...
        if summary_file.exists():
            ...
            num_trajectories = summary.get('evaluation_metadata', {}).get('num_trajectories', 2)
        else:
            ...
            traces_dir = run_path / "traces"
            num_trajectories = len(list(traces_dir.glob("*.json"))) if traces_dir.exists() else 0
        
        # Check if already imported
        existing = conn.execute("SELECT COUNT(*) FROM evaluations WHERE run_id = ?", [run_path.name]).fetchone()[0]
        if existing > 0:
        ...
        # Get environment ID
        env_result = conn.execute("SELECT id FROM environments WHERE name = ?", [env_name]).fetchone()
        if not env_result:
        ...
        """, [
            eval_id, env_id, run_path.name, datetime.now(),
            json.dumps(models_evaluated), json.dumps(difficulties_evaluated),
            num_trajectories, 0.0, 0.0, json.dumps(summary)
        ])
        ...
        for trace_file in trace_files:
                    ...
                    if partitions:
                        ...
                        if events:
                            ...
                            if env_steps:
                                ...
                                outputs = last_env_step.get('compute_output', [{}])[0].get('outputs', {})
                            ...
                            total_achievements = event_metadata.get('total_achievements', 0)
            ...
            """, [
                traj_id, eval_id, trajectory_id,
                trace_metadata.get('model_name', models_evaluated[0] if models_evaluated else 'unknown'),
                trace_metadata.get('difficulty', difficulties_evaluated[0] if difficulties_evaluated else 'unknown'),
                trace_metadata.get('seed'), success, final_reward, num_steps,
...
def cleanup_stale_traces(conn):
    ...
    # Remove stale traces
    if stale_trace_ids:
        conn.execute(f"DELETE FROM traces WHERE id IN ({','.join(['?'] * len(stale_trace_ids))})", stale_trace_ids)
        results.append(f"🧹 Removed {len(stale_trace_ids)} stale trace entries")
    
    # Remove stale trajectories
    if stale_trajectory_ids:
        conn.execute(f"DELETE FROM trajectories WHERE trace_id IN ({','.join(['?'] * len(stale_trajectory_ids))})", stale_trajectory_ids)
        results.append(f"🧹 Removed {len(stale_trajectory_ids)} stale trajectory entries")
    
    # Remove evaluations with no remaining trajectories
    for eval_id in stale_eval_ids:
        remaining_count = conn.execute("SELECT COUNT(*) FROM trajectories WHERE eval_id = ?", [eval_id]).fetchone()[0]
...
def process_crafter_trace(trace_data: Dict) -> Dict[str, Any]:
    ...
    processed = {
        ...
        "total_reward": dataset.get("reward_signals", [{}])[0].get("reward", 0.0) if dataset.get("reward_signals") else 0.0,
        "turns": []
    }
            ...
            if "observation" in outputs:
                ...
                if isinstance(obs, dict):
                    ...
                    if "image" in obs:
                        ...
                        turn_data["images"].append(f"data:image/png;base64,{obs['image_base64']}")
            
            # Extract image directly
            if "image_base64" in outputs:
                turn_data["images"].append(f"data:image/png;base64,{outputs['image_base64']}")
...
def render_crafter_trace(processed_trace: Dict):
    ...
    st.markdown(f"🎮 **Turns:** {len(processed_trace['turns'])} | **Reward:** {processed_trace['total_reward']:.3f} | **Difficulty:** {processed_trace['difficulty']}")
    
    # Achievements section
    if all_achievements:
        st.markdown(f"🏆 **Achievements ({len(all_achievements)}):** " + 
                   " | ".join([f"**{ach.replace('_', ' ').title()}**" for ach in all_achievements]))
    else:
...
def render_nethack_trace(processed_trace: Dict):
    ...
    if st.checkbox("Show All Turns (Compact View)"):
        ...
        for i, turn in enumerate(processed_trace['turns'][:10]):  # Limit to first 10 turns
...
def render_sokoban_trace(processed_trace: Dict):
    ...
    # Ultra-compact info line
    st.markdown(f"🎮 **Turns:** {len(processed_trace['turns'])} | **Boxes Solved:** {processed_trace['boxes_solved']}/{processed_trace['total_boxes']}")
...
def render_minigrid_trace(processed_trace: Dict):
    ...
    if st.checkbox("Show All Steps (Compact View)"):
        ...
        for i, turn in enumerate(processed_trace['turns'][:15]):  # Show more steps for MiniGrid
            ...
            if turn_index < len(partitions):
                ...
                if events:
                    ...
                    if 'agent_compute_step' in event and event['agent_compute_step']:
                        ...
                        # Count messages
                        input_msgs = 0
                        if 'compute_input' in agent_step and agent_step['compute_input']:
                        ...
                        output_msgs = 0
                        if 'compute_output' in agent_step and agent_step['compute_output']:
                            for output_item in agent_step['compute_output']:
                                if 'messages' in output_item:
                                    output_msgs += len(output_item['messages'])
                        
                        agent_summary = f"🔍 **Agent Summary:** {input_msgs} in | {output_msgs} out"
                        ...
                        if 'compute_input' in agent_step and agent_step['compute_input']:
                            with st.expander("📨 Input Messages", expanded=False):
                                for i, input_item in enumerate(agent_step['compute_input']):
                                    if 'messages' in input_item:
                        ...
                        if 'compute_output' in agent_step and agent_step['compute_output']:
                            with st.expander("🤖 Agent Response", expanded=False):
                                for i, output_item in enumerate(agent_step['compute_output']):
                                    if 'messages' in output_item:
                                            ...
                                            if role == 'assistant':
                                                ...
                                                if 'tool_calls' in msg and msg['tool_calls']:
                                                    ...
                                                    for tool_call in msg['tool_calls']:
                                                        ...
                                                        st.code(f"{func_name}({args})", language="json")
                                            
                                            elif role == 'tool':
                                                tool_call_id = msg.get('tool_call_id', 'unknown')
                                                st.error(f"**🛠️ Tool ({tool_call_id}):** {content}")
                                            
                                            else:
                                                st.write(f"**{role.title()}:** {content}")
                    else:
                        st.warning("⚠️ No agent compute step found - this trace is missing agent reasoning!")
                    
                    # Show environment compute steps - collapsible
                    if 'environment_compute_steps' in event and event['environment_compute_steps']:
                        with st.expander("🌍 Environment Response", expanded=False):
                            for i, env_step in enumerate(event['environment_compute_steps']):
                ...
                st.warning(f"Turn {turn_index} not found (only {len(partitions)} partitions available)")
...
def render_trajectory_details(trajectory: Dict, conn=None):
    ...
    if conn:
        ...
        st.info("💡 Full trace visualization will be available when trace data processing is implemented.")
...
def render_crafter_turn(turn_data: Dict):
    ...
    with col1:
        ...
        if turn_data['actions']:
            ...
            action_text = " → ".join([action['name'] for action in turn_data['actions']])
            st.text(action_text)
...
def render_nethack_turn(turn_data: Dict):
    ...
    if turn_data['stats']:
        ...
        with col1:
            st.metric("HP", f"{turn_data['stats']['hp']}/{turn_data['stats']['max_hp']}")
        with col2:
    ...
    if turn_data['actions']:
        st.subheader("Actions")
        action_text = " → ".join([f"{action['name']} ({action['index']})" for action in turn_data['actions']])
        st.text(action_text)
...
def render_nethack_turn_compact(turn_data: Dict):
    ...
    with col1:
        if turn_data['stats']:
            st.write(f"**HP:** {turn_data['stats']['hp']}/{turn_data['stats']['max_hp']}")
    ...
    with col2:
        ...
        st.write(f"**Gold:** {turn_data['stats']['gold']}" if turn_data['stats'] else "")
...
def render_sokoban_turn(turn_data: Dict):
    ...
    action_text = f" | **Action:** {turn_data['action_name']}" if turn_data['action_name'] != "initial" else ""
...
def render_sokoban_turn_compact(turn_data: Dict):
    ...
    with col1:
        ...
        mini_room = '\n'.join(room_lines[:3]) + "..." if len(room_lines) > 3 else turn_data['room_text']
...
def render_minigrid_turn(turn_data: Dict):
    ...
    with col1:
        ...
        if turn_data['images']:
            ...
            for i, img in enumerate(turn_data['images'][:1]):  # Usually one image per step
                try:
...
def render_evaluation_summary(evaluations: List[Dict]):
    ...
    st.dataframe(
        df[['run_id', 'timestamp', 'num_trajectories', 'success_rate', 'avg_achievements']],
...
def main():
    ...
    selected_env_idx = st.sidebar.selectbox("**Select Environment**", range(len(env_names)), format_func=lambda i: env_names[i])
    ...
    if selected_env:
        ...
        for eval_data in all_evaluations:
            ...
            if trajectories:  # Only include evaluations that have trajectories with agent reasoning
                evaluations.append(eval_data)
        
        # Load archived evaluations
        all_archived_evaluations = load_evaluations(conn, selected_env["id"], archived=True)
        ...
        if not evaluations and not archived_evaluations:
            st.warning(f"No evaluations with agent reasoning found for {selected_env['display_name']}")
            st.stop()
        ...
        if total_evals > filtered_evals or total_archived > filtered_archived:
            st.sidebar.markdown(f"ℹ️ **Filtered:** {filtered_evals}/{total_evals} evals, {filtered_archived}/{total_archived} archived with reasoning")
        ...
        if archived_evaluations:
            eval_type_options.append(('archived', f"Archived ({len(archived_evaluations)})"))
        
        if len(eval_type_options) > 1:
            selected_type = st.sidebar.selectbox(
                "**Evaluation Type**",
                options=[opt[0] for opt in eval_type_options],
                format_func=lambda x: next(opt[1] for opt in eval_type_options if opt[0] == x),
                index=0 if st.session_state.evaluation_type == eval_type_options[0][0] else 1
            )
            st.session_state.evaluation_type = selected_type
        else:
            st.session_state.evaluation_type = eval_type_options[0][0] if eval_type_options else 'regular'
        ...
        if trajectories:
            avg_reward = sum(traj['final_reward'] for traj in trajectories) / len(trajectories)
            success_rate = sum(1 for traj in trajectories if traj['success']) / len(trajectories)
            
            # Calculate average achievements (only for trajectories that have achievements)
            achievements_list = [len(traj['achievements']) for traj in trajectories if traj['achievements']]
            avg_achievements = sum(achievements_list) / len(achievements_list) if achievements_list else 0
            
            # Format metrics
            avg_reward_str = f"{avg_reward:.3f}"
            success_rate_str = f"{success_rate:.1%}"
            avg_achievements_str = f"{avg_achievements:.1f}" if avg_achievements > 0 else "N/A"
        else:
            avg_reward_str = "N/A"
            success_rate_str = "N/A"
            avg_achievements_str = "N/A"
        
        st.sidebar.markdown(f"**Trajs:** {len(trajectories)} | **Avg Reward:** {avg_reward_str} | **Success:** {success_rate_str} | **Avg Ach:** {avg_achievements_str}")
        ...
        if not trajectories:
            if total_count > 0:
                st.warning(f"No trajectories with agent reasoning found for selected evaluation (filtered out {total_count} incomplete traces)")
            else:
        ...
        if total_count > filtered_count:
            st.sidebar.markdown(f"ℹ️ **Filtered:** {filtered_count}/{total_count} with reasoning")
