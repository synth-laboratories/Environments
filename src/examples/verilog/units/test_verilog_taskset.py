...
from src.examples.verilog.taskset import (
...
from src.tasks.core import TaskInstanceSet, SplitInfo, Impetus, Intent
...
class TestVerilogTaskset:
    ...
    async def test_create_verilog_taskset_basic(self, mock_load_dataset):
        ...
        assert len(instance.metadata.files_provided) == 3  # TopModule.v, testbench, RefModule.v
    ...
    async def test_create_verilog_taskset_max_instances(self, mock_load_dataset):
        ...
        # Mock larger dataset
        mock_dataset = [{'problem_id': f'test_{i:03d}', 'prompt': f'Test {i}', 'test': '', 'ref': ''} 
                       for i in range(20)]
        mock_load_dataset.return_value = mock_dataset
