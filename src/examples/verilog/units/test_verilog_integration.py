...
from src.examples.verilog.environment import VerilogEnvironment
from src.examples.verilog.taskset import create_verilog_taskset, _create_hf_task_instance
...
class TestVerilogIntegration:
    ...
    async def test_simulation_failure_workflow(self, mock_run, mock_load_dataset):
        ...
        write_call = EnvToolCall(
            tool="write_file",
            args={"path": "TopModule.v", "content": "module TopModule(output zero);\nassign zero = 1'b1;\nendmodule"}  # Wrong logic
        )
        await env.step(write_call)
...
class TestVerilogSystemIntegration:
    ...
    async def test_error_handling_and_recovery(self):
        ...
        write_call = EnvToolCall(
            tool="write_file",
            args={"path": "/invalid/path/file.v", "content": "test"}
        )
    ...
    async def test_concurrent_environments(self, mock_load_dataset):
        ...
        write_calls = [
            env1.step(EnvToolCall(tool="write_file", args={"path": "test1.v", "content": "module test1(); endmodule"})),
            env2.step(EnvToolCall(tool="write_file", args={"path": "test2.v", "content": "module test2(); endmodule"}))
        ]
