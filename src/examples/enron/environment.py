...
class EnronEnvironment(StatefulEnvironment):
    ...
    async def step(
        ...
        public_payload_for_engine = tool_result.payload if tool_result.ok and tool_result.payload else {}
    ...
    async def terminate(self) -> InternalObservation:
        ...
        priv_state_on_terminate = {"reward_last": 0, "total_reward": self.engine.total_reward, "terminated": True, "truncated": False, "gold_answer": self.engine._sample()["answer"]}
        pub_state_on_terminate = {"question": self.engine._sample()["question"], "tools": [], "already_answered": self.engine.answered, "status": "terminated_by_env"}
    ...
    async def checkpoint(self) -> InternalObservation:
        ...
        return {"engine_snapshot": snapshot.model_dump(), "message": "Checkpoint created"}
