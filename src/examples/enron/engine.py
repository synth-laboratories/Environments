...
from examples.enron.art_helpers.email_search_tools import search_emails as helper_search_emails, read_email as helper_read_email, SearchResult
...
class EnronEngine(StatefulEngine):
    ...
    async def _step_engine(
        ...
        pub = {
            "question": s["question"],
            "tools": ["search_emails", "read_email", "answer_question", "terminate"], # Available tools
    ...
    @classmethod
    async def _deserialize_engine(cls, snap: EnronEngineSnapshot, task_instance: EnronTaskInstance) -> "EnronEngine":
        eng = cls(task_instance) 
        eng.idx = snap.idx
        eng.answered = snap.answered
        eng.total_reward = snap.total_reward
        eng.rewards_history = snap.partial_rewards # Ensure this is correctly typed in Pydantic model if not List[float]
...
class EnronStepPenaltyComponent(RewardComponent):
    ...
    async def _calculate_and_apply_reward(self) -> float:
        ...
        reward_context_state = { # State snapshot for reward calculation
            "question": s["question"],
            "gold_answer": s["answer"],
            **(self._current_action_details_for_reward if self._current_action_details_for_reward else {})
        }
        
        # The 'action' param for score can be the conceptual action type or detailed args
        action_param_for_score = self._current_action_details_for_reward if self._current_action_details_for_reward else {}
        
        reward = await self.reward_stack.step_reward(state=reward_context_state, action=action_param_for_score)
    ...
    async def read_email_action(self, message_id: str) -> Optional[Dict[str,Any]]:
        ...
        self._current_action_details_for_reward = {"type": "read", "message_id": message_id}
