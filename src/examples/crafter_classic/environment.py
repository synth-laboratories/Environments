...
class CrafterClassicEnvironment(
    ...
    def __init__(
        ...
        self.custom_checkpoint_observation_callable = custom_ckpt_obs or SynthCrafterObservationCallable()
    ...
    def validate_tool_calls(self, tool_calls: Union[EnvToolCall, List[EnvToolCall], List[List[EnvToolCall]]]) -> EnvToolCall:
        ...
        if isinstance(tool_calls, list):
            ...
            if isinstance(tool_calls[0], list):
                if not tool_calls[0]: raise ValueError("Received empty inner list of tool calls.")
                agent_call = tool_calls[0][0]
            else: agent_call = tool_calls[0]
        elif isinstance(tool_calls, EnvToolCall): agent_call = tool_calls
        else: raise TypeError(f"Unexpected type for tool_calls: {type(tool_calls)}")
    ...
    async def step(self, tool_calls: Union[EnvToolCall, List[EnvToolCall], List[List[EnvToolCall]]]) -> InternalObservation:  # type: ignore[override]
        ...
        if not isinstance(payload_dict, dict) or not tool_result.ok:
            ...
            if tool_result.error: pub_state.error_info = tool_result.error # Should not happen if ok=True
    ...
    @classmethod
    async def _deserialize_engine(cls, snapshot: CrafterEngineSnapshot, task_instance: "CrafterTaskInstance") -> "CrafterClassicEnvironment":
