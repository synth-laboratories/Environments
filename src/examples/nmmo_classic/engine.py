...
class NeuralMMOEngine(StatefulEngine, IReproducibleEngine):
    ...
    def _load_map_data(self) -> None:
        ...
        try:
            ...
            if map_file.exists():
                ...
                logger.info(f"Loaded map data from {map_file}, shape: {self._map_data.shape}")
            else:
        ...
        if h > 256 or w > 256:
            ...
            for i in range(new_h):
                for j in range(new_w):
                    ...
                    region = rgb_image[i*scale_factor:(i+1)*scale_factor, 
    ...
    async def _serialize_engine(self) -> NeuralMMOEngineSnapshot:
        ...
        return NeuralMMOEngineSnapshot(
            ...
            rng_state=getattr(self.env, 'np_random', np.random.default_rng()).bit_generator.state if hasattr(self.env, 'np_random') else {},
        )
