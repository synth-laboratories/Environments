...
from typing import Dict, Any, List, Optional, Deque, Set, Literal
...
try:
    ...
    def patched_validate_messages(self, messages: List[Dict[str, Any]]) -> None:
        ...
        assert all(isinstance(msg, dict) and "content" in msg for msg in messages), "All messages must be dicts with content"
...
from src.examples.red.engine_helpers.reward_library.pallet_town_rewards import (
...
from src.examples.red.engine_helpers.reward_library.exploration_rewards import (
    NewAreaDiscoveryReward,
    BuildingEntryReward
)
from src.examples.red.engine_helpers.reward_library.novelty_rewards import (
    FirstBattleReward,
...
from src.tasks.core import Impetus, Intent, TaskInstance, TaskInstanceMetadata
from src.environment.tools import EnvToolCall
import re

# Import screen analysis functions
from src.examples.red.engine_helpers.screen_analysis import analyze_screen_buffer, create_detailed_screen_description
...
class EarlyGameRewardManager:
    ...
    async def calculate_rewards(self, current_state: Dict[str, Any], prev_state: Dict[str, Any], action_info: Dict[str, Any]) -> float:
        ...
        if total_reward > 0:
            ...
            print(f"[REWARD_TOTAL] Step {current_state.get('step_count', 0)}: +{total_reward:.1f} (Total: {self.total_reward_earned:.1f})")
...
def format_obs_for_llm_from_states(
    pub: PokemonRedPublicState, priv: PokemonRedPrivateState, screen_analysis: dict = None, mode: str = "state_and_screen"
) -> str:
    ...
    if screen_analysis:
        ...
        if mode == "state_and_ascii":
            ...
            if 'colors' in screen_analysis:
                colors_text = "DOMINANT COLORS: " + ", ".join([f"{color}({pct}%)" for color, pct in screen_analysis['colors'].items()])
                screen_description += colors_text + "\n"
            
            # Add entity detection summary
            if 'entities' in screen_analysis:
                screen_description += f"DETECTED ENTITIES: {len(screen_analysis['entities'])} sprite-like objects\n"
...
class PokemonRedHistoryObservationCallable(GetObservationCallable):
    def __init__(self, max_history: int = 1, mode: Literal["state_and_ascii", "state_and_screen"] = "state_and_screen"):
    ...
    async def get_observation(
        ...
        current_state_dict = {
            ...
            "party": [{"species_id": p.species_id, "level": p.level, "hp_current": p.hp_current, "hp_max": p.hp_max} for p in pub.party],
            "inventory": [{"item_id": item.item_id, "quantity": item.quantity} for item in pub.inventory],
        ...
        if self._last_state_hash == current_state_hash and pub.step_count > 1:
            ...
            if self._stuck_count >= 3:
                raise RuntimeError(f"Agent stuck in same state for {self._stuck_count} steps - HARD FAIL. Position: ({pub.player_x}, {pub.player_y}), Map: {pub.map_id}")
        else:
            self._stuck_count = 0
        ...
        try:
            ...
            screen_buffer = env.engine.emulator.screen.ndarray.copy()  # Copy to avoid reference issues
            ...
            print(f"[DEBUG] Screen analysis completed - type: {screen_analysis.get('screen_type', 'UNKNOWN')}")
...
class ReActAgent:
    def __init__(self, llm, max_turns: int = 50):
        ...
        self.valid_buttons = ["A", "B", "UP", "DOWN", "LEFT", "RIGHT", "START", "SELECT"]
    ...
    def _get_recent_reasoning_traces(self, k: int = 5) -> str:
        ...
        for tool_call in tool_calls[-k:]:
            if "tool_arguments" in tool_call and "reasoning" in tool_call["tool_arguments"]:
                step_num = len([e for e in self.history[:self.history.index(tool_call)+1] if e["type"] == "tool_call"])
                reasoning = tool_call["tool_arguments"]["reasoning"]
                buttons = tool_call["tool_arguments"].get("buttons", ["unknown"])
                recent_reasoning.append(f"Step {step_num}: Pressed {buttons} - Reasoning: {reasoning}")
    ...
    async def decide(self, obs_str: str, current_raw_obs: Dict[str, Any], mode: Literal["state_and_ascii", "state_and_screen"] = "state_and_screen") -> List[str]:
        ...
        if mode == "state_and_screen":
            try:
                ...
                if current_raw_obs and 'screen_buffer' in current_raw_obs and current_raw_obs['screen_buffer'] is not None:
                    ...
                    debug_filename = f"step_{current_step_count:04d}_agent_{self.system_instance_id[-8:]}.png"
                    ...
                    print(f"[AGENT_DEBUG] Successfully converted screen to base64 image")
                else:
                    print(f"[AGENT_DEBUG] No screen buffer available in observation")
        ...
        print(f"[AGENT_DEBUG] Calling LLM with prompt length: {len(prompt)}, images: {len(screen_images_bytes)}")
        ...
        try:
            ...
            if not tool_calls:
                print(f"[AGENT_DEBUG] No tool calls found, falling back to A")
                self.history.append(
                    {
                        "type": "tool_call",
            ...
            if len(tool_calls) == 0:
                print(f"[AGENT_DEBUG] Empty tool calls list, falling back to A")
                self.history.append(
            ...
            if (
                ...
                print(f"[AGENT_DEBUG] Unexpected tool_call structure, falling back to A")
                self.history.append(
            ...
            if tool_name == "pokemon_red_interact":
                print(f"[AGENT_DEBUG] Processing pokemon_red_interact tool call")
                validated_args = PokemonRedInteractArgs(**tool_arguments)
                buttons = validated_args.buttons
                print(f"[AGENT_DEBUG] Buttons: {buttons}, Valid: {[button in self.valid_buttons for button in buttons]}")
                ...
                if invalid_buttons:
                    ...
                    self.history.append(
                        {"type": "error", "content": f"Invalid buttons: {invalid_buttons}. Falling back to A."}
                    )
                    return ["A"]
                ...
                print(f"[AGENT_DEBUG] Agent decided to terminate, returning TERMINATE")
                return ["TERMINATE"]
...
@pytest.mark.asyncio
async def test_react_agent_pokemon_red(tmp_path: Path, mode: Literal["state_and_ascii", "state_and_screen"] = "state_and_screen"):
    ...
    inst = PokemonRedTaskInstance(
        ...
        intent=Intent(rubric={"goal": "Collect badges and progress"}, gold_trajectories=None, gold_state_diff={}),
        ...
        for turn in range(agent.max_turns):
            ...
            for i, button in enumerate(buttons):
                ...
                if "error" in obs_payload_next:
                    raise RuntimeError(f"Environment step error on button {i+1}: {obs_payload_next['error']}")
                ...
                if obs_payload["private"].terminated or obs_payload["private"].truncated:
                    print(f"[DEBUG] Environment terminated/truncated after button {i+1}/{len(buttons)}")
                    break
    ...
    dataset = Dataset(
        questions=[
            TrainingQuestion(id="pokemon_red_ep_test", intent="progress_in_game", criteria="completed_episode_or_collected_badges")
        ],
        reward_signals=[
            RewardSignal(
    ...
    assert episode_completed or badges_collected > 0, "Agent failed to complete the episode or collect any badges in the test."
...
async def eval_react_pokemon_red(
    ...
    # ------------------------------------------------------------------ helpers
    async def run_episode_eval(inst: PokemonRedTaskInstance, agent_max_turns: int) -> tuple[bool, int, float, list]:
        ...
        for turn_idx in range(agent.max_turns):
            ...
            if buttons[0] == last_button:
                ...
                if same_button_count >= 8:  # Increased from 4 to 8
                    print(f"[WARNING] Agent pressed same button '{buttons[0]}' {same_button_count} times in a row")
            ...
            print(f"[DEBUG] Environment step completed. Obs keys: {list(obs_payload.keys()) if isinstance(obs_payload, dict) else type(obs_payload)}")
            ...
            if obs_payload.get("screen_buffer") is not None:
                import hashlib
                current_screen_hash = hashlib.md5(obs_payload["screen_buffer"].tobytes()).hexdigest()
                ...
                print(f"[DEBUG] Screen hash: {current_screen_hash[:8]}..., Same count: {run_episode_eval.same_screen_count}, Changed: {screen_changed}")
                ...
                max_same_button = button_tolerance.get(buttons[0], 5)  # Default to 5 for unknown buttons
                ...
                if (run_episode_eval.same_screen_count >= min_screen_unchanged and 
                    turn_idx > min_turn_threshold and 
                    same_button_count >= max_same_button):
                    ...
                    if buttons[0] == 'B':
                        ...
                        if same_button_count < 20:  # Much more lenient for B button
                            print(f"[DEBUG] B button often ineffective in overworld - allowing more attempts ({same_button_count}/20)")
                            # Continue without failing
                            obs_payload = obs_payload_next
                            continue
                    
                    print(f"[WARNING] Agent appears stuck - screen unchanged for {run_episode_eval.same_screen_count} turns with repeated button '{buttons[0]}' {same_button_count} times")
                    print(f"[WARNING] Button tolerance for '{buttons[0]}': {max_same_button}, screen unchanged threshold: {min_screen_unchanged}")
                    raise RuntimeError(f"Agent stuck - screen unchanged for {run_episode_eval.same_screen_count} turns with repeated button '{buttons[0]}' ({same_button_count} times, tolerance: {max_same_button}) - HARD FAIL")
            ...
            if (last_position == current_position and 
                last_map_id == current_map_id and
                not screen_changed and
                turn_idx > 8):  # Much more lenient - allow many turns for dialogue
                stuck_count += 1
                if stuck_count >= 8:  # Require many more turns of true stuck state
                    raise RuntimeError(f"Agent truly stuck - no position or screen changes for {stuck_count} turns. Position: {current_position}, Map: {current_map_id} - HARD FAIL")
            else:
                stuck_count = 0
            ...
            agent.history.append(
                {"type": "tool_response", "content": f"Button sequence executed: {buttons}"}
            )
        ...
        print(f"[DEBUG] Episode result - successful: {run_successful}, badges: {badges_collected}, rewards: {total_rewards:.1f}")
        ...
        return run_successful, badges_collected, total_rewards, hist_cb.reward_manager.reward_history

    # ---------------------------------------------------------------- instance factory
    async def make_pokemon_red_instances(difficulty: str, n_instances: int = 3, start_seed: int = 0) -> List[PokemonRedTaskInstance]:
        instances = []
        ...
        for i in range(n_instances):
            ...
            instance = PokemonRedTaskInstance(
                id=uuid.uuid4(),
                impetus=Impetus(instructions=f"Play Pokemon Red on {difficulty} difficulty and collect badges."),
    ...
    # ---------------------------------------------------------------- evaluation
    configs = [
        ("easy", 1, max_turns),  # (difficulty_label, num_agents/instances, max_turns_per_episode) - Use parameter
    ]
    table_rows = []
    base_seed_for_difficulty = {"easy": 1000, "hard": 2000}

    print(f"Starting Pokemon Red ReAct Agent Evaluation...")
    ...
    for label, num_agents, _ in configs:
        insts = await make_pokemon_red_instances(label, n_instances=num_agents, start_seed=base_seed_for_difficulty[label])
    ...
    for label, num_agents, max_episode_turns in configs:
        ...
        results = await asyncio.gather(*(run_episode_eval(inst, max_episode_turns) for inst in current_difficulty_instances))
        end_time = time.time()
        ...
        table_rows.append([label, f"{num_successful_runs}/{len(current_difficulty_instances)}", f"{avg_badges:.2f}", f"{avg_rewards:.1f}"])
        print(f"Completed {label}: {num_successful_runs}/{len(current_difficulty_instances)} successful, Avg. Badges: {avg_badges:.2f}, Avg. Rewards: {avg_rewards:.1f}")
    ...
    print(
        tabulate(
            table_rows,
            headers=["Difficulty", "Successful Runs", "Avg Badges Collected", "Avg Rewards Earned"],
            tablefmt="github",
        )
    )
    ...
    if all_reward_achievements:
        ...
        reward_summary_rows = []
        for reward_name, count in sorted(all_reward_achievements.items(), key=lambda x: x[1], reverse=True):
...
if __name__ == "__main__":
    ...
    asyncio.run(eval_react_pokemon_red(
        model_name="gpt-4.1-mini",
        max_turns=10,
        mode="state_and_screen"
    ))