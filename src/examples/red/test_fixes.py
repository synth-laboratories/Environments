...
from src.examples.red.taskset import INSTANCE as POKEMON_TASK
...
async def test_environment_setup():
    ...
    try:
        ...
        print(f"Step observation: step_count={obs.get('step_count')}, terminated={obs.get('terminated')}")
...
def test_logging_configuration():
    ...
    jax_loggers = ['jax._src.cache_key', 'jax._src.compilation_cache', 'jax._src.compiler', 'jax._src.dispatch']
    ...
    for logger_name in jax_loggers:
        ...
        if logger.level >= logging.WARNING:
            ...
            print(f"❌ {logger_name} logger level: {logging.getLevelName(logger.level)} (should be WARNING or higher)")
...
def test_safe_compare():
    ...
    from src.examples.red.config_logging import safe_compare
