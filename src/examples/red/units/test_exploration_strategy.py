...
async def test_exploration_when_stuck():
    ...
    inst = PokemonRedTaskInstance(
        ...
        intent=Intent(rubric={"goal": "Find working actions"}, gold_trajectories=None, gold_state_diff={}),
    ...
    try:
        ...
        print(f"[DEBUG] Environment initialized successfully")
        ...
        for button in buttons_to_test:
            ...
            if hasattr(before_pub, 'party_level') and hasattr(after_pub, 'party_level'):
                if before_pub.party_level != after_pub.party_level:
                    state_changes.append(f"party_level: {before_pub.party_level} -> {after_pub.party_level}")
        ...
        effective_buttons = [btn for btn, result in results.items() 
                           if isinstance(result, dict) and result.get("effective", False)]
        
        ineffective_buttons = [btn for btn, result in results.items() 
                             if isinstance(result, dict) and not result.get("effective", False)]
        
        error_buttons = [btn for btn, result in results.items() 
        ...
        if effective_buttons:
            ...
            print(f"  - These buttons cause state changes and may lead to progress")
        else:
            print(f"  - No buttons caused state changes in this initial position")
            print(f"  - May need to investigate game state or save file")
        
        if "LEFT" in effective_buttons or "RIGHT" in effective_buttons:
            print(f"  - Movement is working - agent should explore the area")
        
        if "A" not in effective_buttons:
            print(f"  - 'A' button ineffective at this position - agent needs to move first")
