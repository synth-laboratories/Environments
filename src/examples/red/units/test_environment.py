...
from src.examples.red.engine import (
    GameWorldState, PlayerProgressState, GameSystemState, 
    PokemonData, InventoryItem
)
...
from src.environment.tools import EnvToolCall, ToolResult
...
class TestPokemonRedEnvironment:
    ...
    def mock_engine(self):
        ...
        engine = Mock()
        engine._reset_engine = AsyncMock(return_value=(
            PokemonRedPrivateState(
...
class TestPressButtonTool:
    ...
    def mock_engine(self):
        ...
        engine = Mock()
        engine._step_engine = AsyncMock(return_value=(
            PokemonRedPrivateState(
                reward_last_step=0.0,
                total_reward=0.0,
                terminated=False,
