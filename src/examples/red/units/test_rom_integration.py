...
async def test_game_screen_capture():
    ...
    if hasattr(engine.emulator, 'screen') and hasattr(engine.emulator.screen, 'image'):
        ...
        print(f"✓ Screen capture available: {screen.shape if hasattr(screen, 'shape') else type(screen)}")
    else:
...
async def test_memory_persistence():
    ...
    for button in buttons:
        ...
        state = engine._extract_current_state()
        print(f"  After {button}: map_id={state['map_id']}, pos=({state['player_x']},{state['player_y']})")
...
async def test_environment_integration():
    ...
    for button, frames in title_screen_sequence:
        ...
        print(f"  {button}: pos={obs['position']}, step={obs['step_count']}, reward={obs['total_reward']:.3f}")
