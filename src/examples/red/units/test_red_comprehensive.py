...
from src.examples.red.taskset import INSTANCE
...
async def test_memory_state_tracking():
    ...
    critical_metrics = [
        "map_id", "player_x", "player_y", "badges", "in_battle", 
        "party_level", "party_hp_current", "party_hp_max", "party_xp"
    ]
    ...
    print(f"✓ State after button press - some values may change")
...
async def test_observation_richness():
    ...
    expected_fields = [
        "position", "badges_earned", "badges_bitfield", "hp_status",
        "party_level", "party_xp", "in_battle", "step_count",
        "reward_last_step", "total_reward", "terminated"
    ]
...
async def test_game_progression_detection():
    ...
    from src.examples.red.engine_helpers.reward_components import BadgeRewardComponent
    badge_component = BadgeRewardComponent()
    
    reward = await badge_component.score(
        state=current_state,
        action={"prev_badges": prev_state["badges"]}
    )
