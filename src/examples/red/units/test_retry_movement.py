...
async def test_movement_with_retry():
    ...
    inst = PokemonRedTaskInstance(
        ...
        intent=Intent(rubric={"goal": "Move left reliably"}, gold_trajectories=None, gold_state_diff={}),
    ...
    try:
        ...
        for button, expected_behavior in movement_tests:
            ...
            if movement_occurred:
                print(f"[SUCCESS] {button} movement worked! Position: {before_position} -> {after_position}")
                if map_changed:
                    print(f"[NOTICE] Map also changed: {before_map} -> {after_map}")
                successful_movements += 1
            else:
                print(f"[WARNING] {button} movement had no effect. Position stayed: {after_position}")
        ...
        if successful_movements > 0:
            print("[SUCCESS] Engine retry mechanism is working - at least some movements succeeded!")
        else:
