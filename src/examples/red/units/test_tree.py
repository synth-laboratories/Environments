...
from src.examples.red.taskset import PokemonRedTaskInstance, INSTANCE as DEFAULT_TASK
from src.examples.red.environment import PokemonRedEnvironment
from src.examples.red.engine import PokemonRedEngineSnapshot
...
async def pokemon_red_mcts_plan(
    ...
    *,
    ...
    for depth in range(max_depth):
        ...
        # Log current state
        priv, pub = env.engine._create_states(reward=0.0)
        LOG.debug(f"State: Map{pub.map_id:02X}:({pub.player_x},{pub.player_y}) "
        ...
        for action in POKEMON_ACTIONS:
            ...
            child_env = await PokemonRedEnvironment._deserialize_engine(
                pickle.loads(gzip.decompress(tree.load_snapshot_blob(child_id))), DEFAULT_TASK
            )
            ...
            if rollout_scores:
                ...
                LOG.debug(f"Action {action}: Q={q_vals[action]:.3f} "
...
async def test_mcts_pokemon_red_execution(tmp_path: Path) -> None:
    ...
    LOG.debug(f"Initial state - Score: {initial_score:.3f}, "
    ...
    LOG.debug(f"Final state - Score: {final_score:.3f}, "
    ...
    progress_made = (
        final_pub.map_id != initial_pub.map_id or
        final_pub.player_x != initial_pub.player_x or
        final_pub.player_y != initial_pub.player_y or
        final_pub.party_level > initial_pub.party_level or
        final_pub.badges != initial_pub.badges or
        abs(final_score - initial_score) > 0.01
    )
...
async def test_heuristic_functions() -> None:
    ...
    LOG.debug(f"Heuristic tests - Initial: {initial_score:.3f}, "
