...
async def test_menu_close_bug_reproduction():
    ...
    try:
        ...
        print(f"Initial position: ({obs_payload['public'].player_x}, {obs_payload['public'].player_y})")
        ...
        for press_num in range(1, max_presses + 1):
            ...
            new_position = (step_result['public'].player_x, step_result['public'].player_y)
            screen_changes = step_result.get("screen_changes", 0)
            
            print(f"  Menu state: {menu_states[-1]} -> {new_menu_state}")
            print(f"  Screen hash: {screen_hashes[-1][:8] if screen_hashes[-1] else None}... -> {new_screen_hash[:8] if new_screen_hash else None}...")
            ...
            if new_menu_state != menu_states[-2]:
                print(f"  ✓ Menu state changed!")
            else:
                print(f"  ✗ Menu state unchanged")
            
            # Check if screen changed
            if new_screen_hash != screen_hashes[-2]:
                print(f"  ✓ Screen changed!")
            else:
                print(f"  ✗ Screen unchanged")
            
            # If we're in a "stuck" scenario like the agent, break early
            if press_num >= 5 and new_menu_state == menu_states[-2] and new_screen_hash == screen_hashes[-2]:
        ...
        if initial_menu_state and initial_menu_state != 0:
            ...
            if final_menu_state == initial_menu_state and len(unique_screen_hashes) <= 2:
                ...
                print(f"   - After {len(menu_states) - 1} B presses, menu state: {final_menu_state}")
...
async def test_menu_close_bug_fix_verification():
    ...
    try:
        ...
        final_position = (step_result['public'].player_x, step_result['public'].player_y)
        screen_changes = step_result.get("screen_changes", 0)
        ...
        print(f"✅ ANALYSIS COMPLETE: B button behavior is correct")
        print(f"   Initial menu state: {initial_menu_state}")
        print(f"   Final menu state: {final_menu_state}")
        
        if final_menu_state == initial_menu_state:
            print(f"✅ EXPECTED: Menu state unchanged (menu_state=1 is normal overworld state)")
            print(f"   This indicates the B button correctly does nothing when no menu is open")
        else:
            print(f"⚠️ UNEXPECTED: Menu state changed when none was expected to be open")
            print(f"   This might indicate an actual menu was open and closed")
...
async def test_engine_direct_button_retry():
    ...
    try:
        ...
        if initial_menu_state != 0:
            ...
            if final_menu_state != initial_menu_state:
                print(f"✅ Direct engine test SUCCESS: Menu closed!")
                print(f"   Menu state: {initial_menu_state} -> {final_menu_state}")
                print(f"   Method returned: {success}")
            else:
                print(f"❌ Direct engine test FAILURE: Menu didn't close")
...
async def test_low_level_button_debug():
    ...
    from src.examples.red.engine_helpers.memory_map import MENU_STATE
    ...
    try:
        ...
        from src.examples.red.engine import BUTTON_MAP
        ...
        from src.examples.red.engine_helpers.memory_map import (
            MAP_ID, PLAYER_X, PLAYER_Y, TEXT_BOX_ACTIVE, WARP_FLAG
        )
...
async def test_menu_state_investigation():
    ...
    inst = PokemonRedTaskInstance(
        ...
        intent=Intent(rubric={"goal": "Understand menu state"}, gold_trajectories=None, gold_state_diff={}),
        metadata=task_metadata,
        is_reproducible=True,
        initial_engine_snapshot=None
    )
    ...
    try:
        from src.examples.red.engine_helpers.memory_map import (
            MENU_STATE, MAP_ID, PLAYER_X, PLAYER_Y, TEXT_BOX_ACTIVE
        )
...
async def test_comprehensive_menu_interaction():
    ...
    inst = PokemonRedTaskInstance(
        ...
        intent=Intent(rubric={"goal": "Understand menu type"}, gold_trajectories=None, gold_state_diff={}),
        metadata=task_metadata,
        is_reproducible=True,
        initial_engine_snapshot=None
    )
...
async def test_movement_away_from_interface():
    ...
    inst = PokemonRedTaskInstance(
        ...
        intent=Intent(rubric={"goal": "Exit interface by moving"}, gold_trajectories=None, gold_state_diff={}),
        metadata=task_metadata,
        is_reproducible=True,
        initial_engine_snapshot=None
    )
    ...
    try:
            ...
            for attempt in range(3):
                ...
                if new_pos != initial_pos:
                    ...
                    if memory[0xCC28] == 0 or (memory[0xCC25] == 0 and memory[0xCC24] == 0):
                        print(f"  🎉 MENU STATE CLEARED! Interface closed by moving away.")
                        return
                    else:
                        print(f"  Menu state still persistent after movement")
                    break
                else:
        ...
        if final_menu_last == 3 and final_cursor[0] > 0:
            ...
            print(f"The 'menu state' appears to be:")
            print(f"1. A persistent overworld UI element")
            print(f"2. Possibly related to the game's initial state")
            print(f"3. Not an actual menu that can/should be closed with B")
            print(f"4. Normal game behavior in this location")
            
            print(f"\n💡 SOLUTION for the agent:")
            print(f"1. Don't treat menu_state=1 as 'menu is open'")
            print(f"2. Remove the B button retry logic for 'menu closing'")
            print(f"3. Focus on actual game progression instead")
            print(f"4. Use movement and A button for interactions")
        else:
