...
class PressButtonInput(BaseModel):
    button: str = Field(..., description="Game Boy button: A, B, UP, DOWN, LEFT, RIGHT, START, SELECT")
...
class PokemonRedObservationCallable(GetObservationCallable):
    async def get_observation(self, pub: PokemonRedPublicState, priv: PokemonRedPrivateState) -> InternalObservation:
        """Convert Pokemon Red states to agent observation"""
        from .engine_helpers.state_extraction import get_badge_count, format_position, format_hp_status
...
class PokemonRedEnvironment(StatefulEnvironment, ReproducibleEnvironment[PokemonRedEngine]):
    ...
    def __init__(
        ...
        self.custom_checkpoint_observation_callable = custom_ckpt_obs or PokemonRedObservationCallable()
    ...
    async def terminate(self) -> InternalObservation:
        ...
        obs_dict = {"terminated": True, "message": "Pokemon Red environment terminated."}
    ...
    @classmethod
    async def _deserialize_engine(cls, snapshot: PokemonRedEngineSnapshot, task_instance: PokemonRedTaskInstance) -> "PokemonRedEnvironment":
