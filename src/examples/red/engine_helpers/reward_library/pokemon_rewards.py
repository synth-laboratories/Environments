...
class RarePokemonCaughtReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        for pokemon in party:
            species_id = pokemon.get("species_id", 0)
            if (species_id in self.rare_species and 
                species_id not in self.rare_pokemon_caught):
...
class EvolutionStonePokemonReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        for pokemon in party:
            species_id = pokemon.get("species_id", 0)
            if (species_id in self.evolution_stone_pokemon and 
                species_id not in self.evolution_stone_pokemon_caught):
...
class TypeCollectionReward(RewardComponent):
    ...
    def __init__(self):
        ...
        self.pokemon_types = {
            1: "grass", 4: "fire", 7: "water", 25: "electric",
            # Add more mappings
        }
