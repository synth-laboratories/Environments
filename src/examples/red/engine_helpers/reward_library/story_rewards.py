...
from src.environment.rewards.core import RewardComponent
...
class RivalBattleCompletionReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        if (prev_in_battle and not current_in_battle and battle_outcome == 1 and
            current_map in self.rival_battle_maps and 
            current_map not in self.rival_battles_completed):
...
class LegendaryEncounterReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        if (current_map in self.legendary_maps and 
            current_map not in self.legendary_encounters):
...
class SafariZoneSuccessReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        if (prev_map in self.safari_zone_maps and 
            current_map not in self.safari_zone_maps):
...
class FossilRevivalReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        for pokemon in party:
            ...
            if (species_id in self.fossil_pokemon and 
                species_id not in self.fossils_revived):
