...
class GymLeaderVictoryReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        # Victory in a gym
        if (prev_in_battle and not current_in_battle and 
            battle_outcome == 1 and current_map in self.gym_maps):
...
class EliteFourMemberVictoryReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        if (prev_in_battle and not current_in_battle and 
            battle_outcome == 1 and current_map in self.elite_four_maps):
...
class ChampionVictoryReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        if (prev_in_battle and not current_in_battle and 
            battle_outcome == 1 and current_map == self.champion_map):
