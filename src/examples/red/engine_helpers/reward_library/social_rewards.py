...
class StoryDialogueProgressionReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        if (state["text_box_active"] and not action.get("prev_text_box_active", False) and
            location in story_locations and location not in self.story_npcs_talked_to):
...
class ProfessorOakInteractionsReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        if (state["map_id"] == 3 and state["text_box_active"] and 
            not action.get("prev_text_box_active", False)):
...
class NPCGiftReceivedReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        if (state["text_box_active"] and 
            (current_inventory_count > prev_inventory_count or 
             current_party_count > prev_party_count)):
            return 15.0
        return 0.0
...
class NameRaterUsageReward(RewardComponent):
    ...
    async def score(self, state: Dict[str, Any], action: Dict[str, Any]) -> float:
        ...
        if (location == name_rater_location and state["text_box_active"] and
            not action.get("prev_text_box_active", False)):
