...
from src.environment.rewards.core import RewardStack
...
class PokemonRedEngine(StatefulEngine, IReproducibleEngine):
    ...
    def __init__(self, task_instance: TaskInstance, skip_rom_check: bool = False):
        ...
        if not skip_rom_check:
            ...
            if not rom_path.exists():
                raise FileNotFoundError(f"Pokemon Red ROM not found at {rom_path}. Please see README.md for setup instructions.")
    ...
    def _press_button_with_retry(self, button: str, frames: int = 1, max_attempts: int = 10) -> bool:
        """
        ...
        if button in movement_buttons:
            ...
            try:
                ...
                initial_position = (initial_state.get("player_x", 0), initial_state.get("player_y", 0))
            ...
            for attempt in range(max_attempts):
                ...
                try:
                    ...
                    new_position = (new_state.get("player_x", 0), new_state.get("player_y", 0))
                    ...
                    if new_position != initial_position or new_map != initial_map:
                        logging.debug(f"Movement successful after {attempt + 1} attempts: {initial_position} -> {new_position}")
                        return True
                        
                except Exception as e:
                    logging.warning(f"Could not extract state during movement retry attempt {attempt + 1}: {e}")
                    continue
            
            # If we get here, movement didn't occur after max_attempts
            logging.warning(f"Movement button {button} pressed {max_attempts} times but no position change detected")
            return False
    ...
    def _create_states(self, reward: float, terminated: bool = False) -> tuple[PokemonRedPrivateState, PokemonRedPublicState]:
    ...
    async def _reset_engine(self, *, seed: Optional[int] = None) -> tuple[PokemonRedPrivateState, PokemonRedPublicState]:
        """Reset the Pokemon Red engine to initial state"""
        # Load initial save state if provided
        if hasattr(self.task_instance, 'initial_engine_snapshot') and self.task_instance.initial_engine_snapshot:
    ...
    async def _step_engine(self, action: Dict[str, Any]) -> tuple[PokemonRedPrivateState, PokemonRedPublicState]:
        ...
        try:
            ...
            try:
                ...
                logging.error(f"Error checking termination condition with badges={current_state.get('badges')}: {e}")
                terminated = False
    ...
    @classmethod 
    async def _deserialize_engine(cls, snapshot: PokemonRedEngineSnapshot, task_instance: TaskInstance) -> "PokemonRedEngine":
        ...
        engine._previous_state = {k: v for k, v in snapshot.state_data.items() if k != '_save_state_bytes'}
