...
TERMINATION_REASONS = ["timeout", "goal_reached", "agent_quit", "environment_error", "lava_death"]
...
def minigrid_balrog_score(achievements_unlocked: int, task_completion_rate: float, 
                         avg_efficiency: float) -> float:
    """
    MiniGrid BALROG scoring based on:
    - Achievement unlocking (40%)
    - Task completion rate (40%) 
    - Movement efficiency (20%)
    """
    achievement_score = (achievements_unlocked / len(ALL_ACHIEVEMENTS)) * 40
    completion_score = task_completion_rate * 40
    efficiency_score = avg_efficiency * 20
    return achievement_score + completion_score + efficiency_score

def minigrid_hafner_score(task_success_rates: List[float]) -> float:
    """Hafner-style log-adjusted score across task types."""
    if not task_success_rates:
        return 0.0
    N = len(task_success_rates)
    g = sum(math.log(1 + s) for s in task_success_rates) / N
    return math.exp(g) - 1
...
class MiniGridTrajectoryResult:
    ...
    efficiency_ratio: float          # steps_to_goal / optimal_steps
    ...
    termination_reason: str  # "timeout", "goal_reached", "agent_quit", "environment_error", "lava_death"
...
class MiniGridEvalFramework:
    ...
    async def run_single_trajectory(
        ...
        from synth_env.examples.minigrid.agent_demos.test_synth_react import MiniGridReActAgent
        from synth_env.examples.minigrid.environment import MiniGridEnvironment
        from synth_env.examples.minigrid.taskset import create_minigrid_taskset, MiniGridTaskInstance, MiniGridTaskInstanceMetadata
        ...
        try:
            while turn_count < max_turns:
                ...
                # Track achievements
                easy_count = len([a for a in achievements_unlocked if a in MINIGRID_ACHIEVEMENTS["basic"]])
                medium_count = len([a for a in achievements_unlocked if a in MINIGRID_ACHIEVEMENTS["intermediate"]])
                hard_count = len([a for a in achievements_unlocked if a in MINIGRID_ACHIEVEMENTS["advanced"]])
                ...
                pbar.set_postfix({
                    ...
                    "steps": obs_payload.get("public", {}).step_count if hasattr(obs_payload.get("public", {}), "step_count") else 0
                })
                ...
                new_achievements = self._check_achievements(obs_payload, achievements_unlocked, turn_count, task_type)
                ...
                action_decision = await agent.decide(current_formatted_obs, task_description, turn_count)
                
                if action_decision.get("name") == "terminate":
                ...
                if collect_detailed_data:
                    turn_data = {
                        ...
                        "steps_before_turn": obs_payload.get("public", {}).step_count if hasattr(obs_payload.get("public", {}), "step_count") else 0
                    }
                ...
                if obs_payload.get("private", {}).terminated or obs_payload.get("private", {}).truncated:
                    if "lava" in obs_payload.get("formatted_obs", "").lower():
                        termination_reason = "lava_death"
                    elif obs_payload.get("public", {}).terminated:
    ...
    def _check_achievements(self, obs_payload: Dict[str, Any], current_achievements: Set[str], 
                          turn: int, task_type: str) -> Set[str]:
        ...
        if "first_door_open" not in current_achievements and "door" in formatted_obs and "open" in formatted_obs:
        ...
        if "navigate_empty_room" not in current_achievements and "empty" in task_type.lower():
            if obs_payload.get("private", {}).terminated:
    ...
    def _convert_action_format(self, action_decision: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action_decision.get("name") == "minigrid_act":
            action = action_decision.get("parameters", {}).get("action", "forward")
    ...
    def _generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "achievement_percentage_table": self._generate_achievement_percentage_table(grouped_results),
    ...
    def _compute_aggregate_metrics(self, model_name: str, difficulty: str, 
                                 trajectories: List[MiniGridTrajectoryResult]) -> MiniGridAggregateResults:
        """Compute aggregate metrics for a model-difficulty condition."""
        
        num_trajectories = len(trajectories)
        if num_trajectories == 0:
            return MiniGridAggregateResults(
                model_name=model_name, difficulty=difficulty, num_trajectories=0,
                success_rate=0.0, avg_total_steps=0.0, avg_total_turns=0.0, avg_total_reward=0.0,
                task_completion_rates={}, avg_efficiency_ratio=0.0, avg_exploration_coverage=0.0,
                avg_wall_collisions=0.0, avg_backtrack_count=0.0,
                unique_achievements_unlocked=set(), total_achievement_count=0, 
                avg_achievements_per_trajectory=0.0, achievement_unlock_rates={}, hafner_score=0.0,
                balrog_score_avg=0.0, balrog_score_best=0.0,
                avg_actions_per_turn_overall=0.0, actions_per_turn_distribution={},
                termination_breakdown={}, avg_final_position=None
            )
        ...
        avg_exploration_coverage = sum(t.exploration_coverage for t in trajectories) / num_trajectories
        ...
        # Compute Hafner adjusted score
        task_success_rates = list(task_completion_rates.values())
        hafner_adjusted_score = minigrid_hafner_score([rate * 100 for rate in task_success_rates])
        
        # Compute BALROG scores
        balrog_scores = [minigrid_balrog_score(len(traj.achievements_unlocked), 
                                             1.0 if traj.success else 0.0, 
                                             traj.efficiency_ratio) for traj in trajectories]
        ...
        avg_actions_per_turn_overall = (
            sum(all_actions_per_turn) / len(all_actions_per_turn) 
            if all_actions_per_turn else 0.0
        )
    ...
    def _generate_summary_table(self, aggregate_results: List[MiniGridAggregateResults]) -> pd.DataFrame:
    ...
    def _generate_achievement_percentage_table(self, grouped_results: Dict[str, Dict[str, List[MiniGridTrajectoryResult]]]) -> pd.DataFrame:
    ...
    def _generate_task_completion_table(self, aggregate_results: List[MiniGridAggregateResults]) -> pd.DataFrame:
    ...
    def _generate_navigation_analysis(self, aggregate_results: List[MiniGridAggregateResults]) -> pd.DataFrame:
        ...
        for agg in aggregate_results:
            data.append({
                ...
                "Final Position": f"({agg.avg_final_position[0]:.1f}, {agg.avg_final_position[1]:.1f})" if agg.avg_final_position else "N/A"
            })
    ...
    def _generate_trajectory_breakdown(self) -> pd.DataFrame:
        ...
        for traj in self.trajectory_results:
            ...
            easy_achievements = len([a for a in traj.achievements_unlocked if a in MINIGRID_ACHIEVEMENTS["basic"]])
            medium_achievements = len([a for a in traj.achievements_unlocked if a in MINIGRID_ACHIEVEMENTS["intermediate"]])
            hard_achievements = len([a for a in traj.achievements_unlocked if a in MINIGRID_ACHIEVEMENTS["advanced"]])
            ...
            data.append({
                ...
                "Final Position": f"({traj.final_position[0]}, {traj.final_position[1]})" if traj.final_position else "N/A",
                "Achievements": ", ".join(sorted(traj.achievements_unlocked)) if traj.achievements_unlocked else "None"
            })
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        if not achievement_df.empty:
            ...
            for category in ["basic", "intermediate", "advanced"]:
                category_cols = ["Model", "Difficulty"] + [col for col in achievement_df.columns 
                                                         if col in MINIGRID_ACHIEVEMENTS[category]]
        ...
        sample_cols = ["Model", "Difficulty", "Task Type", "Success", "Steps", "Total Achievements", "Termination"]
