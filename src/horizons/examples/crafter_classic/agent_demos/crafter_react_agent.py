...
from .environments.examples.crafter_classic.environment import (
    CrafterClassicEnvironment,
    CrafterPublicState,
    CrafterPrivateState,
)
from .environments.examples.crafter_classic.engine import (
...
from .environments.examples.crafter_classic.taskset import (
    CrafterTaskInstance,
        ...
        if tool_name == "crafter_interact":
            ...
            for action_str in actions_list:
                if action_str in ACTION_STRING_TO_INT:
                    ...
                    print(
                        f"[WARNING] Invalid action '{action_str}', using noop instead"
                    )
        ...
        for turn in range(agent.max_turns):
            ...
            for act_idx in action_sequence:
                ...
                if (
                    ...
                    player_health = priv_state.player_internal_stats.get(
                        "health", "N/A"
                    )
...
async def eval_react_crafter(
    ...
    from .environments.examples.crafter_classic.agent_demos.eval_framework import (
        run_crafter_eval,
    )
