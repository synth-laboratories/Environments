...
from horizons.environments.examples.minigrid.environment_mapping import (
    get_difficulty_from_seed,
    get_environment_from_seed,
    validate_environment_name,
)
from horizons.environments.reproducibility.core import IReproducibleEngine
...
class MiniGridEngine(StatefulEngine, IReproducibleEngine):
    ...
    def get_front_cell_debug(self) -> Dict[str, Any]:
        """Return detailed information about the cell in front of the agent."""
        unwrapped = self.env.unwrapped
        fwd_pos = getattr(unwrapped, "front_pos", None)
        info: Dict[str, Any] = {"front_pos": None, "in_bounds": False, "cell": None}
        if fwd_pos is None:
            return info

        info["front_pos"] = tuple(fwd_pos)
        in_bounds = 0 <= fwd_pos[0] < unwrapped.width and 0 <= fwd_pos[1] < unwrapped.height
        info["in_bounds"] = in_bounds
        if not in_bounds:
            info["cell"] = {"type": "boundary", "blocks_movement": True}
            return info

        cell = unwrapped.grid.get(*fwd_pos)
        if cell is None:
            info["cell"] = {"type": "empty", "blocks_movement": False}
        else:
            # Map to simple debug fields
            ctype = getattr(cell, "type", "unknown")
            ccolor = getattr(cell, "color", None)
            cstate = getattr(cell, "state", None) if hasattr(cell, "state") else None
            blocks = ctype in {"wall", "lava"}
            info["cell"] = {
                "type": ctype,
                "color": ccolor,
                "state": cstate,
    ...
    def get_agent_debug(self) -> Dict[str, Any]:
        """Return basic agent debug information (position and direction)."""
        unwrapped = self.env.unwrapped
        pos = tuple(unwrapped.agent_pos) if unwrapped.agent_pos is not None else None
        dir_int = int(unwrapped.agent_dir) if hasattr(unwrapped, "agent_dir") else None
        dir_symbols = ["→", "↓", "←", "↑"]
        dir_symbol = dir_symbols[dir_int] if dir_int is not None else None
        return {"agent_pos": pos, "agent_dir": dir_int, "agent_dir_symbol": dir_symbol}
