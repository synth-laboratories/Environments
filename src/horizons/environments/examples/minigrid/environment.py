...
from horizons.environments.environment.tools import AbstractTool, EnvToolCall, ToolResult
from horizons.environments.examples.minigrid.engine import (
...
class MiniGridActionInput(BaseModel):
    ...
    Supports either a single action via 'action' or a sequence of actions via
    'actions'. Exactly one of these should be provided.
    """

    action: Optional[str] = Field(
        None,
        description="Single action to take. One of: 'left', 'right', 'forward', 'pickup', 'drop', 'toggle', 'done'",
    )
    actions: Optional[List[str]] = Field(
        None,
        description="Sequence of actions to execute consecutively. Each must be one of: 'left', 'right', 'forward', 'pickup', 'drop', 'toggle', 'done'",
    )
...
class MiniGridInteractTool(AbstractTool):
    ...
    async def __call__(self, call: EnvToolCall) -> ToolResult:
        ...
        try:
            ...
            actions_arg = call.args.get("actions")
            if actions_arg is not None:
                if not isinstance(actions_arg, list) or not actions_arg:
                    return ToolResult(ok=False, error="'actions' must be a non-empty list of action names", payload={})

                executed = []
                last_priv: Optional[MiniGridPrivateState] = None
                last_pub: Optional[MiniGridPublicState] = None

                for idx, act_name in enumerate(actions_arg):
                    if not isinstance(act_name, str):
                        return ToolResult(ok=False, error=f"Invalid action at index {idx}: must be string", payload={})
                    action_name = act_name.lower()
                    ...
                    action_int = self.action_map[action_name]
                    last_priv, last_pub = await self.engine._step_engine(action_int)
                    executed.append(action_name)

                    # Stop early if episode ends
                    if last_priv.terminated or last_priv.truncated:
                        break

                # Safety: ensure states exist
                assert last_priv is not None and last_pub is not None

                parts = [f"Executed actions: {executed}."]
                if last_priv.reward_last != 0:
                    parts.append(f"Last Reward: {last_priv.reward_last:.2f}")
                if last_priv.terminated:
                    parts.append("Episode terminated!")
                    if last_priv.info.get("success", False):
                        parts.append("Mission completed successfully!")
                elif last_priv.truncated:
                    parts.append("Episode truncated (max steps reached).")

                return ToolResult(
                    ok=True,
                    payload={
                        "message": " ".join(parts),
                        "public_state": last_pub,
                        "private_state": last_priv,
...
class MiniGridEnvironment(StatefulEnvironment, ReproducibleEnvironment[MiniGridEngine]):
    ...
    async def step(
        ...
        if result.ok:
            ...
            # Include executed_actions in observation if provided
            extra: Dict[str, Any] = {}
            if "executed_actions" in result.payload:
                extra["executed_actions"] = result.payload["executed_actions"]
            debug_extra = self._debug_extras()
            extra.update(debug_extra)
            return await self._to_observation(
                priv, pub, self.custom_step_observation_callable, extra_obs=extra
            )
        else:
            ...
            return await self._to_observation(
                ...
                pub,
                self.custom_step_observation_callable,
                extra_obs={"error": result.error, **self._debug_extras()},
            )
    ...
    async def checkpoint(self) -> InternalObservation:
        ...
        priv, pub = self.engine.get_current_states_for_observation()
        return await self._to_observation(
            priv, pub, self.custom_checkpoint_observation_callable, extra_obs=self._debug_extras()
        )
    ...
    async def _deserialize_engine(cls, data: Dict[str, Any]) -> MiniGridEngine:
        ...
        from horizons.environments.examples.minigrid.engine import MiniGridEngineSnapshot
    ...
    def _debug_extras(self) -> Dict[str, Any]:
        """Assemble rich debug extras describing engine and immediate surroundings."""
        try:
            front = self.engine.get_front_cell_debug()
            agent = self.engine.get_agent_debug()
            actions = self.engine.get_available_actions()
            return {
                "env_name": self.engine.env_name,
                "seed": self.engine.seed,
                "difficulty": self.engine.difficulty,
                "front_cell": front,
                "agent_debug": agent,
                "available_actions": [name for (_i, name) in actions],
            }
        except Exception:
            return {}
