...
from horizons.environments.examples.sokoban.taskset import (
    SokobanTaskInstance,
...
class SokobanEngine(StatefulEngine, IReproducibleEngine):
    ...
    async def _serialize_engine(self) -> SokobanEngineSnapshot:
        ...
        if hasattr(self.task_instance, 'serialize') and callable(self.task_instance.serialize):
            task_instance_dict = await self.task_instance.serialize()
        else:
            # Fallback for SimpleNamespace objects - convert to dict
            task_instance_dict = vars(self.task_instance)
