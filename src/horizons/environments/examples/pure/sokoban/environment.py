...
from horizons.environments.examples.sokoban.engine import (
    SokobanEngine,
...
class SokobanEnvironment(StatefulEnvironment, ReproducibleEnvironment[SokobanEngine]):
    ...
    def validate_tool_calls(
        ...
        print(f"🐍 SOKOBAN: validate_tool_calls called with: {tool_calls}", file=sys.stderr)
        print(f"🐍 SOKOBAN: tool_calls type: {type(tool_calls)}", file=sys.stderr)
        if isinstance(tool_calls, list):
            print(f"🐍 SOKOBAN: tool_calls is list, length: {len(tool_calls)}", file=sys.stderr)
            if not tool_calls:
                raise ValueError("Received empty list of tool calls.")
            first_item = tool_calls[0]
            print(f"🐍 SOKOBAN: first_item type: {type(first_item)}", file=sys.stderr)
            if isinstance(first_item, list):
                ...
                print(f"🐍 SOKOBAN: Found EnvToolCall instance: {first_item}", file=sys.stderr)
                ...
                print(f"🐍 SOKOBAN: About to raise TypeError for type: {type(first_item)}", file=sys.stderr)
            ...
            print(f"🐍 SOKOBAN: tool_calls is direct EnvToolCall: {tool_calls}", file=sys.stderr)
    ...
    async def checkpoint(self) -> InternalObservation:
        ...
        if isinstance(obs_data, dict):
            ...
            obs_data["engine_snapshot_data"] = (
                dataclasses.asdict(engine_snapshot)
