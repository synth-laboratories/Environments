...
ALL_ACHIEVEMENTS = [
    ach for category in ACHIEVEMENT_CATEGORIES.values() for ach in category
]
...
class AggregateResults:
    ...
    achievement_unlock_rates: Dict[
        str, float
    ]  # achievement -> % of trajectories that unlocked it
...
class CrafterEvalFramework:
    ...
    def _generate_comprehensive_report(self) -> Dict[str, Any]:
        ...
        for model_name, difficulties in grouped_results.items():
            for difficulty, trajectories in difficulties.items():
                agg = self._compute_aggregate_metrics(
                    model_name, difficulty, trajectories
                )
        ...
        report = {
            ...
            "multi_action_analysis": self._generate_multi_action_analysis(
                aggregate_results
            ),
            "trajectory_by_trajectory_breakdown": self._generate_trajectory_breakdown(),
            "model_comparison_tables": self._generate_model_comparison_tables(
                aggregate_results
            ),
            ...
            "raw_trajectory_results": [
                asdict(traj) for traj in self.trajectory_results
            ],
        }
    ...
    def _compute_aggregate_metrics(
        ...
        balrog_scores = [
            balrog_score(len(traj.achievements_unlocked)) for traj in trajectories
        ]
        balrog_score_avg = (
            sum(balrog_scores) / len(balrog_scores) if balrog_scores else 0.0
        )
        ...
        termination_breakdown = {
            reason: count / num_trajectories
            for reason, count in termination_counts.items()
        }

        # Survival stats
        health_values = [
            t.final_health for t in trajectories if t.final_health is not None
        ]
        food_values = [t.final_food for t in trajectories if t.final_food is not None]
        drink_values = [
            t.final_drink for t in trajectories if t.final_drink is not None
        ]
        ...
        avg_final_drink = (
            sum(drink_values) / len(drink_values) if drink_values else None
        )
        ...
        for model_name, difficulties in grouped_results.items():
            for difficulty, trajectories in difficulties.items():
                ...
                for achievement in ALL_ACHIEVEMENTS:
                    ...
                    percentage = (
                        count / num_trajectories if num_trajectories > 0 else 0.0
                    )
            ...
            summary_data.append(
                {
                    ...
                    "Distribution": str(
                        dict(sorted(agg.actions_per_turn_distribution.items()))
                    ),
                }
            )
                ...
                for model_name, difficulties in grouped_results.items():
                    for difficulty, trajectories in difficulties.items():
                        ...
                        count = sum(
                            1
                            for traj in trajectories
                            if achievement in traj.achievements_unlocked
                        )
                        percentage = (
                            count / num_trajectories if num_trajectories > 0 else 0.0
                        )
        ...
        for agg in aggregate_results:
            ...
            if agg.model_name in MODEL_NAME_TO_SOTA:
                ...
                if sota_name in BALROG_SOTA_SCORES["balrog_leaderboard"]:
                    balrog_sota_score = BALROG_SOTA_SCORES["balrog_leaderboard"][
                        sota_name
                    ]
                    percentage_of_balrog_sota_avg = (
                        agg.balrog_score_avg / balrog_sota_score
                    ) * 100
                    percentage_of_balrog_sota_best = (
                        agg.balrog_score_best / balrog_sota_score
                    ) * 100
                    balrog_row["% of BALROG SOTA (Avg)"] = (
                        f"{percentage_of_balrog_sota_avg:.1f}%"
                    )
                    balrog_row["% of BALROG SOTA (Best)"] = (
                        f"{percentage_of_balrog_sota_best:.1f}%"
                    )
                    balrog_row["BALROG SOTA Reference"] = (
                        f"{sota_name} ({balrog_sota_score:.1f}%)"
                    )
        ...
        for agg in aggregate_results:
            ...
            if agg.model_name in MODEL_NAME_TO_SOTA:
                ...
                if sota_name in BALROG_SOTA_SCORES["balrog_leaderboard"]:
                    ...
                    percentage_of_balrog_sota = (
                        agg.balrog_score_avg / balrog_sota_score
                    ) * 100
                    row["% of BALROG SOTA"] = f"{percentage_of_balrog_sota:.1f}%"
                    row["BALROG SOTA Reference"] = (
                        f"{sota_name} ({balrog_sota_score:.1f}%)"
                    )
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        for category in ["Easy", "Medium", "Hard"]:
            category_data = achievement_summary[
                achievement_summary["Category"] == category
            ]
        ...
        if "% of BALROG SOTA (Avg)" in balrog_clean.columns:
            balrog_clean = balrog_clean.rename(
                columns={"% of BALROG SOTA (Avg)": "% SOTA Avg"}
            )
        if "% of BALROG SOTA (Best)" in balrog_clean.columns:
            balrog_clean = balrog_clean.rename(
                columns={"% of BALROG SOTA (Best)": "% SOTA Best"}
            )
