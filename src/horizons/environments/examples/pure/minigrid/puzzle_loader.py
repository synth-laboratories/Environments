...
from horizons.environments.examples.minigrid.environment_mapping import ENVIRONMENT_MAPPING
...
class MiniGridPuzzleLoader:
    ...
    def load_puzzles(self) -> None:
        ...
        # Load puzzles for each difficulty (predefined mapping)
        ...
        # Optionally load persisted ultra_ultra_easy puzzles from JSON
        try:
            from pathlib import Path
            import json

            json_path = Path(__file__).parent / "ultra_ultra_easy_puzzles.json"
            if json_path.exists():
                with open(json_path, "r") as f:
                    data = json.load(f)
                uue = data.get("puzzles", {}).get("ultra_ultra_easy", [])
                if uue:
                    self.puzzles["ultra_ultra_easy"] = []
                    for p in uue:
                        puzzle = MiniGridPuzzle.from_dict(p)
                        self.puzzles["ultra_ultra_easy"].append(puzzle)
                        self.all_puzzles.append(puzzle)
                    logger.info(
                        f"Loaded {len(self.puzzles['ultra_ultra_easy'])} ultra_ultra_easy puzzles from {json_path}"
                    )
        except Exception as e:
            logger.warning(f"Failed to load ultra_ultra_easy persisted puzzles: {e}")
    ...
    def get_available_difficulties(self) -> List[str]:
        """Get list of available difficulty levels (including persisted ones)."""
        # Ensure loaded, then return keys from puzzles dict for completeness
        if not self._loaded:
            self.load_puzzles()
