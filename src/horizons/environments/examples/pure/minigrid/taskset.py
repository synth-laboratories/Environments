...
from horizons.environments.examples.minigrid.puzzle_loader import (
    MiniGridPuzzle,
    get_puzzle_by_seed,
    get_puzzle_loader,
)
...
async def create_minigrid_taskset(
    ...
    """Generate MiniGrid task instances deterministically via concrete seeds.

    This uses a curated set of deterministic seeds per difficulty rather than
    random sampling, ensuring repeatable tasksets across runs.

    Args:
        num_tasks_per_difficulty: Number of tasks to generate for each difficulty.
            Defaults to {"easy": 10, "medium": 10, "hard": 10}
        seed: Optional offset to rotate through the deterministic seed lists
    ...
    # Use the curated deterministic seeds from the puzzle loader
    loader = get_puzzle_loader()
    loader.load_puzzles()

    # Map to available difficulties in the loader
    difficulty_name_map = {
        "easy": "easy",
        "medium": "medium",
        "hard": "hard",
    ...
    for difficulty, num_tasks in num_tasks_per_difficulty.items():
        if difficulty not in difficulty_name_map:
            continue

        loader_diff = difficulty_name_map[difficulty]
        puzzles = loader.get_puzzles_by_difficulty(loader_diff)
        if not puzzles:
            continue

        # Deterministic offset if seed provided
        start_idx = 0 if seed is None else (seed % len(puzzles))

        for i in range(num_tasks):
            idx = (start_idx + i) % len(puzzles)
            puzzle = puzzles[idx]
            instance = await create_minigrid_task_from_puzzle(puzzle)
    ...
    # Deterministic split: rotate but do not shuffle
    if seed is not None and n_total > 0:
        rot = seed % n_total
        instances = instances[rot:] + instances[:rot]
