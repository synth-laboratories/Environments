...
import collections
import logging, crafter
_logger = logging.getLogger(__name__)
_logger.debug("Applying Crafter deterministic patch (deterministic ordering)")

# -----------------------------------------------------------------------------
# 1.  Make per–chunk object order stable
# -----------------------------------------------------------------------------
if not hasattr(crafter.Env, "_orig_balance_object"):
    _logger.debug("Patching crafter.Env._balance_object")
    ...
    _logger.debug("crafter.Env._balance_object patched")
else:
    _logger.debug(
        "crafter.Env._balance_object already patched or _orig_balance_object exists"
    )
...
if not hasattr(crafter.engine.World, "_orig_objects_prop"):
    crafter.engine.World._orig_objects_prop = (
        crafter.engine.World.objects
    )  # save original
