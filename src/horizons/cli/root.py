#!/usr/bin/env python3
"""
Canonical CLI entrypoint for Horizons AI.
"""
...
@cli.command()
@click.option("--db-file", default="traces/v3/horizons.db", help="Database file path")
...
@click.option(
    "--reload/--no-reload",
    default=False,
    help="Enable auto-reload (default: off). Or set HORIZONS_RELOAD=1",
)
@click.option(
    "--force/--no-force",
    default=True,
...
def serve(
        ...
        for proc in processes:
            if proc.poll() is None:
                ...
                try:
                    ...
                    proc.kill()
        # Don't call sys.exit here - let the main loop handle cleanup naturally
    ...
    if not no_env:
        click.echo("")
        click.echo(f"🚀 Starting Horizons AI Environment Service on port {env_port}")
        click.echo("")
        ...
        reload_enabled = reload or (os.getenv("HORIZONS_RELOAD", "0") == "1")
        click.echo(
        ...
        if reload_enabled:
            ...
            if os.path.exists("src/horizons"):
                uvicorn_cmd.extend(["--reload-dir", "src/horizons"])
    ...
    if processes:
        ...
        try:
            ...
            # Signal handler already handled the shutdown
            pass
        finally:
